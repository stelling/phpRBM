...
//	echo("<h2>Werk in uitvoering. Graag niet gebruiken.</h2>");
	...
	if ($_SESSION['webmaster'] == 0 and $zv != 0 and in_array($zv, $lg) === false and in_array($_SESSION['settings']['mailing_alle_zien'], $lg) === false) {
		...
		if (count($rows) > 0) {
			$kols[0]['link'] = sprintf("index.php?tp=%s&amp;op=histdetails&mhid=%%d", $_GET['tp']);
			$kols[0]['class'] = "details";
			$kols[0]['columnname'] = "RecordID";
			
			$kols[1]['headertext'] = "Verzonden";
			$kols[1]['columnname'] = "Verzonden";
			$kols[1]['type'] = "datetime";
			
			$kols[2]['headertext'] = "Aan";
			$kols[2]['columnname'] = "Aan";
			$kols[2]['type'] = "combitext";
...
class Mailing {
	...
	public function add_del_selectie($actie="aantal", $p_ondid=0, $p_vangebdatum="", $p_temgebdatum="") {
		...
		$i_base = new cls_db_base();
		if ($ondid > 0) {
			$f = sprintf("EL.RecordID=%d", $ondid);
			$elid = $i_el->max("RecordID", $f);
			if ($elid > 0) {
...
function lijstmailings($p_filter="") {
	...
	if (isset($_GET['MailingHistID']) and $_GET['MailingHistID'] > 0 and $_GET['op'] == "bekijkmail") {
		...
		if ($p_filter == "Outbox") {
			$kols[1]['headertext'] = "Wacht vanaf";
			$kols[1]['columnname'] = "Ingevoerd";
			
			$kols[5]['link'] = sprintf("index.php?tp=%s&op=edit_email&MailingHistID=%%d", urlencode($_GET['tp']));
			...
			$kols[7]['headertext'] = "Versturen na";
			$kols[7]['columnname'] = "NietVersturenVoor";
			$kols[7]['type'] = "DTHMS";
...
class email {
	...
	public function to_outbox($p_direct=0) {
		...
		if (strlen($this->aanadres) <= 5 and $this->lidid > 0) {
		...
		if ($this->vanafid <= 0) {
		...
		} elseif (strlen($this->aanadres) == 0) {
			debug("geen aanadres aanwezig", 1, 1);
			
		} elseif(isValidMailAddress($this->aanadres) == false and strpos($this->aanadres, ",") == false) {
			$mess = sprintf("Aanadres '%s' is geen geldig e-mailadres.", $this->aanadres);
			debug($mess, 1, 1);
			...
			if ($p_direct == 1 or $_SESSION['settings']['mailing_direct_verzenden'] == 1) {
				$this->nietversturenvoor = date("Y-m-d H:i:s", strtotime("-30 minute"));
			} else {
...
function fnMailingInstellingen() {
	...
	$kols[4]['headertext'] = "Gewijzigd";
	$kols[4]['columnname'] = "Gewijzigd";
	$kols[4]['type'] = "datetime";
	$kols[4]['readonly'] = true;
	$kols[4]['type'] = "datetime";
	$kols[5]['headertext'] = "&nbsp;";
	
	$kols[5]['link'] = sprintf("%s?tp=%s/%s&delete_vanaf=%%d", $_SERVER['PHP_SELF'], $currenttab, $currenttab2);
	$kols[5]['columnname'] = "RecordID";
...
function eigennotificatie($p_ondid, $p_aanadres, $p_tas=-1, $p_interval=24, $p_cc="", $p_onderwerp="", $p_vanafid=-1) {
	...
	$f = sprintf("TypeActiviteit=4 AND TypeActiviteitSpecifiek=%d", $p_tas);
	if ($i_lb->max("DatumTijd", $f) < date("Y-m-d H:i:s", strtotime(sprintf("-%d hour", $p_interval)))) {
		if ($i_ond->oid > 0) {
			$ng = $i_ond->naam;
