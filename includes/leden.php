...
function fnLedenlijst() {
	...
	if ($currenttab2 == "Nieuw (klos)lid") {
		...
		$kols[9]['sortcolumn'] = "LM.Lidnr";
		$kols[10]['sortcolumn'] = "LM.LIDDATUM";
		$kols[11]['sortcolumn'] = "LM.Opgezegd";
		...
		if ($currenttab2 == "Klosleden") {
			...
			$kols[8] = ['columnname' => "RecordID", 'headertext' => "Ond.", 'type' => "subqry", 'class' => "onderdelen", 'subqry' => $sq];			
			
		} else {
			if ($toonlidnummer == 1) {
				$kols[9] = ['columnname' => "Lidnr", 'headertext' => "Lidnummer", 'type' => "integer", 'sortcolumn' => "LM.Lidnr"];
			}
			if ($toonvanaf == 1) {
				$kols[10] = ['columnname' => "LIDDATUM", 'headertext' => "Lid vanaf", 'type' => "date", 'sortcolumn' => "LM.LIDDATUM"];
			}
			if ($toonopgezegd == 1) {
				$kols[11] = ['columnname' => "Opgezegd", 'headertext' => "Opgezegd per", 'type' => "date", 'sortcolumn' => "LM.Opgezegd"];
			}
			if ($currenttab2 == "Leden") {
	...
	if ($lidid > 0) {
		...
		if ($actie == "Algemene gegevens") {
			...
			if (strlen($mm) > 0) {
				...
				foreach (explode(",", $mm) as $kodesoort) {
					...
					if (isset(ARRSOORTMEMO[$kodesoort])) {
						$curval = $i_Mm->inhoud($lidid, $kodesoort);
						printf("<label class='form-label'>%s</label><textarea cols=85 rows=10 name='%s' OnChange='this.form.submit();'>%s</textarea>\n", ARRSOORTMEMO[$kodesoort], $namevar, $curval);
					}
				}
				echo("</form>\n");
			} else {
			...
			if ($_SERVER['REQUEST_METHOD'] == "POST") {
				...
				<label class='form-label'>Login:</label><input type='text' title='Login' name='ingelogdlogin' value='%s' readonly='readonly'>
				<label class='form-label'>Oude wachtwoord:</label><input type='password' name='pw_oud' title='Huidige wachtwoord' maxlength=12>
				<label class='form-label'>Nieuw wachtwoord:</label><input type='password' name='pw_nieuw' title='Nieuw wachtwoord' maxlength=12>
				<label class='form-label'>Herhaal wachtwoord:</label><input type='password' name='pw_herhaal' title='Herhaal nieuw wachtwoord' maxlength=12>", $_SESSION['username']);
				printf("</fieldset>
				%s
...
function fnNieuwLid() {
	...
	if ($_SERVER['REQUEST_METHOD'] == "POST") {
		...
		if (isset($_POST['insid']) and $_POST['insid'] > 0) {
			foreach (array("Datum", "Naam", "Opmerking", "EersteLes", "Verwerkt", "LidID") as $cntr) {
				if (isset($_POST[$cntr])) {
					$i_ins->update($_POST['insid'], $cntr, $_POST[$cntr]);
				}
			}
		}
	}
	...
	if (isset($_GET['op']) and $_GET['op'] == "edit" and $_GET['RecordID'] > 0 and toegang("editinschrijving", 1, 1)) {
		...
		if (strlen($ops) > 0) {
			printf("<label id='lblselecteerinschrijving' class='form-label'>of selecteer inschrijving</label><select name='inschrijving' class='form-select' title='Selecteer inschrijving'><option value=0>Geen</option>\n%s</select>\n", $ops);
		}

		if (toegang("Ledenlijst/Wijzigen lid/Lidmaatschap", 0, 0) == true) {
			echo("<label class='form-label'>Lid vanaf</label><input type='date' name='lidvanaf' title='Lid vanaf'><p>(Indien ingevuld, dan wordt het lidmaatschap toegevoegd)</p>\n");
		}
		...
		if (count($insrows) > 0) {
			$kols[0] = array('columnname' => "RecordID", 'headertext' => "#");
...
function editinschrijving($p_insid) {
	...
	printf("<form method='post' action='%s?tp=%s' id='inschrijvingmuteren'>\n", $_SERVER["PHP_SELF"], $_GET['tp']);
...
function algemeenlidmuteren($lidid) {
	...
	$u = "";
	if (strlen($row->Postcode) == 0) {
		$u = "";
	} elseif (strlen($row->Postcode) < 6) {
	...
	echo("<div id='opdrachtknoppen'>\n");
	printf("<button type='submit' class='%s'>%s Controleer gegevens</button>\n", CLASSBUTTON, ICONCHECK);
	if ($sl == "K") {
		if ($row->Verwijderd > "1900-01-01") {
			echo("<button type='submit' name='Undo_Verwijderen'>Verwijderen ongedaan maken</button>\n");
		} else {
			printf("<button type='submit' class='%s' name='Kloslid_Verwijderen'>%s Kloslid</button>\n", CLASSBUTTON, ICONVERWIJDER);
		}
	}
	echo("</div> <!-- Einde opdrachtknoppen -->\n");
	echo("</form>\n");
...
//			$script .= sprintf("$('#%1\$s').datepicker();\n$('#%1\$s').datepicker('dateFormat', 'yy-mm-dd').datepicker('maxDate', new Date());\n$('#%1\$s').datepicker('setDate', '%3\$s').datepicker('option', 'gotoCurrent', true);\n", $id, $minbehaald, $ldrow->DatumBehaald);
	...
	printf("<select name='Extra' class='form-select' OnChange='this.form.submit();'><option value=-1>Toevoegen ...</option>%s</select>%s\n", $i_dp->htmloptions(0, -1, $zs, getvar("inclVervallen", 0)), $vf);
...
function fnPersoonlijkeAgenda() {
	...
	for ($sw=$dtStart;$sw <= strtotime("+22 day");$sw=strtotime("+1 day", $sw)) {
		$hd = date("Y-m-d", $sw);
		...
		foreach ((new cls_Evenement())->lijst(4, date("Y-m-d", $sw)) as $row) {
			$i_ed->vulvars($row->RecordID, $_SESSION['lidid']);
			$o = $row->Omschrijving;
		
			if ($i_ed->onlineinschrijving == 1 and toegang("Zelfservice/Evenementen", 0, 0)) { 
				$kalitem[$in]['datum'] = substr($row->Datum, 0, 10) . " " . $i_ed->starttijd;
				$o = sprintf("<a href='%s/index.php?tp=Zelfservice/Evenementen'>%s</a>", BASISURL, $o);
				if (strlen($i_ed->status) > 0) {
					$o .= " (" . $i_ed->status . ")";
				}
				$kalitem[$in]['oms'] = $o;
				$in++;
				...
				if ($i_ed->edid > 0) {
					$o .= ": " . $i_ed->status;
					if ($i_ed->aantal > 1) {
						$o .= sprintf(" met %d personen", $i_ed->aantal);
					} elseif (strlen($i_ed->functie) > 0) {
	...
	if ($_SESSION['lidid'] > 0 and  $_SESSION['settings']['termijnvervallendiplomasmelden'] > 0) {
		...
		foreach ($rows as $row) {
			if ($row->VervaltPer >= date("Y-m-d", strtotime("-7 days"))) {
				$kalitem[$in]['datum'] = date("Y-m-d", strtotime($row->VervaltPer));
...
function jubilarissen() {
	...
	echo("<div class='clear'></div>\n");
	printf("<label class='form-label'>Groep met kader</label><select id='kaderonderdeelid' class='form-select' onChange='saveparam(this);'><option value=-1>Geen</option>\n%s</select>\n", $i_ond->htmloptions($_SESSION['settings']['kaderonderdeelid'], 1, "G"));
