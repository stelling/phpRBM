...
function fnLedenlijst() {
	...
	$i_lo->auto_einde(-1, 360);
	$i_lo->autogroepenbijwerken(-1, 360);
...
function fnNieuwLid() {
	...
	if (isset($_GET['op']) and $_GET['op'] == "edit" and $_GET['RecordID'] > 0 and toegang("editinschrijving", 1, 1)) {
		...
		printf("<button type='submit' class='%s btn-sm' name='addkloslid' value='Toevoegen'>%s (Klos)lid</button>\n", CLASSBUTTON, ICONTOEVOEGEN);
...
function editinschrijving($p_insid) {
	...
	if ($currenttab == "Ledenlijst") {
		printf("<label class='form-label'>Gekoppeld aan lid</label><select id='LidID' name='LidID' class='form-select form-select-sm'>\n<option value=0>Geen</option>\n%s</select>\n", $i_lid->htmloptions($i_ins->lidid, 7));
			...
			if (strlen($i_ins->xml) >= 100) {
				...
				foreach ($xml as $col => $val) {
					if ($col == "Achternaam" or $col == "GEBDATUM" or $col == "Email") {
						// Deze kolommen uit de XML overslaan, omdat ze ook in de tabel staan.
...
function onderdelenmuteren($ondtype="G") {
	...
	if ($ondtype == "C") {
		...
		$ondnaammv = ARRTYPEONDERDEEL[$ondtype];
	} elseif ($ondtype == "R") {
		$ondnaammv = "Rollen";
	} elseif ($ondtype == "S") {
		$ondnaammv = "Selecties";
	...
	if ($ondtype != "A" and $ondtype != "C" and $ondtype != "S") {
		fnDispMenu(2);
	}
...
//	echo("<h2>Werk in uitvoering, er mag getest worden.</h2>\n");
	...
	if (isset($_POST['NieuwOnderdeel'])) {
		$onderdeelid = $i_ond->add($ondtype);
	}
	$i_ond->vulvars($onderdeelid, $ondtype);
	...
	if ($scherm == "L") {
		...
		$kols[] = array('headertext' => "Naam", 'columnname' => "Naam");
		if ($ondtype != "M") {
			$kols[3] = array('headertext' => "Kader?", 'columnname' => "Kader", 'type' => "checkbox", 'readonly' => true);
		}
		...
		if ($ondtype == "M") {
			...
			printf("<button type='submit' class='%s' name='NieuwOnderdeel'>%s %s</button>\n", CLASSBUTTON, ICONTOEVOEGEN, $i_ond->typeoms);
			echo("</form>\n");
		}
...
function detailsonderdeelmuteren($p_ondid) {
	...
	if ($i_ond->type != "T" and $i_ond->type != "M" and $i_ond->type != "O") {
	...
	$i_ond->where = "";
	if ($i_ond->type == "A") {
		printf("<label for='LIDCB' class='form-label'>Contributie lid</label><input id='LIDCB' type='text' class='bedrag' value='%.2F'>", $i_ond->contributielid);
		printf("<label for='JEUGDCB' class='form-label'>Contributie jeugdlid</label><input id='JEUGDCB' type='text' class='bedrag' value='%.2F'>", $i_ond->contributiejeugdlid);
		printf("<label for='FUNCTCB' class='form-label'>Contributie afdelingsfunctionaris</label><input id='FUNCTCB' class='bedrag' type='text' value='%.2F'>", $i_ond->contributiekaderlid);
		printf("<label for='NIETLIDCB' class='form-label'>Contributie niet-lid</label><input id='NIETLIDCB' type='text' class='bedrag' value='%.2F'>", $i_ond->contributienietlid);
		printf("<label for='MogelijkeParttimeperc' class='form-label'>Mogelijke parttimepercentages</label><input id='MogelijkeParttimeperc' class='w30' type='text' value='%s' maxlength=30>", $i_ond->mogelijkeparttimeperc);
	}
		
	printf("<label for='vervallenper' class='form-label'>Vervallen per</label><input type='date' id='VervallenPer' value='%s'>\n", $i_ond->vervallen);
	printf("<label class='form-label'>Historie opschonen</label><input type='number' class='num3' id='HistorieOpschonen' value=%d><p>dagen na beëindiging</p>\n", $i_ond->historieopschonen);
	if ($i_ond->type == "A") {
	...
	if (($i_ond->type == "G" or $i_ond->type == "R" or $i_ond->type == "S") and WEBMASTER) {
		...
		if (strlen($i_ond->mysql) > 10 and $i_ond->controleersql($i_ond->mysql, 1) == false) {
			echo("<p class='waarschuwing'>Deze code kan niet worden uitgevoerd.</p>");
		} elseif (strlen($i_ond->mysql) > 10) {
			$st = microtime(true);
			$result = (new cls_db_base())->execsql($i_ond->mysql);
	...
	printf("<label class='form-label'>Laatst bijgewerkt</label><p>%s</p>", $i_ond->laatstemutatie($i_ond->oid, 2, "/"));
...
function fnEigenGegevens($lidid=0) {
	...
	$i_foto = new cls_Foto(-1, $i_lid->lidid);
	$i_ond = new cls_Onderdeel();
	$i_lo = new cls_Lidond();
	...
	if ($lidid <= 0) {
		...
		$naamlid = htmlentities($i_lid->naam);
		$gs = $i_lid->geslacht;
		...
		if (toegang($ct . $tn, 0, 0)) {
			...
			$outp = "<div id='overzichtlid'>\n<h2>Persoonsgegevens</h2>\n";
			$outp .= sprintf("<label class='form-label'>Naam</label><p>%s</p>\n", $i_lid->naam);
			$outp .= sprintf("<label class='form-label'>Voorletters</label><p>%s</p>\n", $i_lid->voorletters);
			$outp .= sprintf("<label class='form-label'>Gender</label><p>%s</p>\n", $i_lid->geslachtoms);
			$outp .= sprintf("<label class='form-label'>Geboren</label><p>%s</p>\n", $i_lid->geboren);
			if ($i_lid->leeftijd > 1) {
				$outp .= sprintf("<label class='form-label'>Leeftijd</label><p>%d jaar</p>\n", $i_lid->leeftijd);
			}
			
			$outp .= "<h2>Adresgegevens</h2>\n";
			$outp .= sprintf("<label class='form-label'>Adres</label><p>%s</p>\n", $i_lid->adres);
			$outp .= sprintf("<label class='form-label'>Postcode en woonplaats</label><p>%s %s</p>\n", $i_lid->postcode, $i_lid->woonplaats);
			
			$outp .= "<h2>Contactgegevens</h2>\n";
			$outp .= sprintf("<label class='form-label'>Telefoon</label><p>%s</p>\n", $i_lid->telefoon);
			$outp .= sprintf("<label class='form-label'>E-mail</label><p>%s</p>\n", $i_lid->email);
			if (strlen($i_lid->emailouders) > 5 and $i_lid->email != $i_lid->emailouders) {
				$outp .= sprintf("<label class='form-label'>E-mail ouders</label><p>%s</p>\n", $i_lid->emailouders);
			}
			if (strlen($i_lid->emailvereniging) > 5 and $i_lid->email != $i_lid->emailvereniging) {
				$outp .= sprintf("<label class='form-label'>E-mail vereniging</label><p>%s</p>\n", $i_lid->emailvereniging);
			}
			if (strlen($i_lid->waarschuwenbijnood) > 0) {
				$outp .= sprintf("<label class='form-label'>Waarschuwen bij nood</label><p>%s</p>\n", $i_lid->waarschuwenbijnood);
			}

			if ($i_lid->soortlid != "Kloslid") {
				$outp .= "<h2>Lidmaatschap</h2>\n";
				if ($i_lid->soortlid != "Lid") {
					$outp .= sprintf("<label class='form-label'>Soort lid</label><p>%s</p>\n", $i_lid->soortlid);
				}
				if ($i_lid->lidnr > 0) {
					$outp .= sprintf("<label class='form-label'>Lidnummer</label><p>%d</p>\n", $i_lid->lidnr);
				}
				$outp .= sprintf("<label class='form-label'>Lid vanaf</label><p>%s</p>\n", $dtfmt->format(strtotime($i_lid->lidvanaf)));
				if ($i_lid->eindelaatstelidmaatschap < "2199-12-31") {
					$outp .= sprintf("<label class='form-label'>Lid t/m</label><p>%s</p>\n", $dtfmt->format(strtotime($i_lid->eindelaatstelidmaatschap)));
				}
				if ($i_lid->lengtelidmaatschap > 0) {
					$outp .= sprintf("<label class='form-label'>Lengte lidmaatschap</label><p>%.1f jaar</p>\n", $i_lid->lengtelidmaatschap);
				}
			}
			...
			$outp .= sprintf("<label class='form-label'>Login</label><p>%s</p>\n", $i_lid->login);
			if (strlen($i_lid->vogafgegeven) >= 10) {
				$outp .= sprintf("<label class='form-label'>VOG afgegeven</label><p>%s</p>\n", $dtfmt->format(strtotime($i_lid->vogafgegeven)));
			}
			if (strlen($i_lid->beroep) > 0) {
				$outp .= sprintf("<label class='form-label'>Beroep</label><p>%s</p>\n", $i_lid->beroep);
			}

			$outp .= "<h2>Recordinformatie</h2>\n";
			$outp .= sprintf("<label class='form-label'>RecordID</label><p>%d</p>\n", $i_lid->lidid);
			$outp .= sprintf("<label class='form-label'>Ingevoerd</label><p>%s</p>\n", $i_lid->laatstemutatie($i_lid->lidid, 1, " / "));
			$outp .= sprintf("<label class='form-label'>Gewijzigd</label><p>%s</p>\n", $i_lid->laatstemutatie($i_lid->lidid, 2, " / "));
			
			$fd = $i_foto->fotodata;
			if ($fd != null) {
				$outp .= sprintf("<div id='pasfoto'><img src='%s' title='Laatst gewijzigd op %s'></div>\n", $fd, $dtfmt->format(strtotime($i_foto->laatstgewijzigd)));
			}
			
			if ((new cls_Onderdeel())->Aantal("Type='E'") > 0 and toegang($ct . $tn, 0, 0)) {
				$rows = (new cls_Lidond())->overzichtlid($i_lid->lidid, "E");
				if (count($rows) > 0) {
				}
			}
			$outp .= "</div>\n";
		...
		$tn = "Materiaal";
		$rows = $i_lo->overzichtlid($lidid, "M");
		$kols = null;
		if (count($rows) > 0 and toegang($ct . $tn, 0, 0)) {
			$kols[] = array('headertext' => "Omschrijving", 'columnname' => "Naam");
			$kols[] = array('headertext' => 'Opmerking', 'columnname' => 'Opmerk');
		...
		if ($i_lid->geslacht != "B" and count($i_lo->lijstperlid($lidid, "T")) > 0 and toegang($ct . $tn, 0, 0)) {
		...
		if (count($rows) > 0 and toegang($ct . $tn, 0, 0)) {
			...
			foreach($rows as $row) {
				...
				if ($i_ed->lidid == $_SESSION['lidid']) {
					if ($i_ed->i_ev->datum >= date("Y-m-d") and $i_ed->aanwezig) {
...
function algemeenlidmuteren($lidid) {
	...
	$sl = substr($i_lid->soortlid, 0, 1);
		
	$wijzvelden[] = array('label' => "Roepnaam", 'naam' => "Roepnaam", 'lengte' => 17);
	$wijzvelden[] = array('label' => "Voorletter(s)", 'naam' => "Voorletter", 'lengte' => 10, 'nietverw' => true, 'titel' => "Voorletter(s)");
	$wijzvelden[] = array('label' => "Tussenvoegsel(s)", 'naam' => "Tussenv", 'lengte' => 7, 'titel' => "Tussenvoegsel(s)");
	$wijzvelden[] = array('label' => "Achternaam", 'naam' => "Achternaam", 'lengte' => 45, 'nietverw' => true);
	if (strlen($i_lid->meisjesnaam) > 0) {
		$wijzvelden[] = array('label' => "Meisjesnaam", 'naam' => "Meisjesnm", 'lengte' => 25, 'titel' => "Meisjesnaam, wordt niet getoond");
	}
	$wijzvelden[] = array('label' => "Gender", 'naam' => "Geslacht", 'nietverw' => true, 'titel' => "Gender");
	if ($i_lid->geslacht != "B") {
		$wijzvelden[] = array('label' => "Geboortedatum", 'naam' => "GEBDATUM", 'type' => 'date', 'titel' => "Geboortedatum");
		$wijzvelden[] = array('label' => "Geboorteplaats", 'naam' => "GEBPLAATS", 'lengte' => 22, 'titel' => "Geboorteplaats");
	}
	...
	$wijzvelden[] = array('label' => "Vast telefoonnummer", 'naam' => "Telefoon", 'lengte' => 21, 'titel' => "Vast telefoonnummer");
	$wijzvelden[] = array('label' => "Mobiele nummer", 'naam' => "Mobiel", 'lengte' => 21, 'titel' => "Mobiele telefoonnummer");
	if ($i_lid->geslacht == "B") {
		$wijzvelden[] = array('label' => "E-mail", 'naam' => "Email", 'lengte' => 45);
	} elseif ($i_lid->leeftijd >= 8) {
		$wijzvelden[] = array('label' => "E-mail", 'naam' => "Email", 'lengte' => 45, 'titel' => "Persoonlijke e-mailadres van het lid");
	}
	if (($_SESSION['settings']['zs_incl_emailvereniging'] == 1 or $currenttab2 == "Wijzigen lid") and $i_lid->leeftijd >= 12 and $sl != "K") {
	...
	if (($_SESSION['settings']['zs_incl_emailouders'] == 1 or $currenttab2 == "Wijzigen lid") and $i_lid->geslacht != "B") {
		if (strlen($i_lid->emailouders) > 5 or $i_lid->leeftijd < 18) {
			$wijzvelden[] = array('label' => "E-mail ouders", 'naam' => "EmailOuders", 'lengte' => 75, 'titel' => "E-mailadres van de ouder(s)");
		}
	}
	if (($i_lid->geslacht != "B" and $i_lid->leeftijd < 18) or strlen($i_lid->namenouders) > 0) {
		$wijzvelden[] = array('label' => "Namen ouders", 'naam' => "NamenOuders", 'lengte' => 90, 'titel' => "Namen van de ouder(s)");
	}
	if ($i_lid->geslacht != "B") {
...
//	printf("<form method='post' id='%s' class='form-check form-switch' action='%s'>\n", __FUNCTION__, $actionurl);
	...
	for ($i=0; $i < count($wijzvelden); $i++) {
		...
		if (($wijzvelden[$i]['naam'] == "Adres" or $wijzvelden[$i]['naam'] == "Woonplaats") and strlen($i_lid->postcode) >= 6) {
		
			$pdok = pdok($i_lid->postcode);
	...
	printf("<label class='form-label'>Ingevoerd</label><p>%s</p>", $i_lid->laatstemutatie($i_lid->lidid, 1, " / "));
	printf("<label class='form-label'>Laatst gewijzigd</label><p>%s</p>", $i_lid->laatstemutatie($i_lid->lidid, 2, " / "));
...
function toestemmingenmuteren($lidid) {
	...
	echo("<div id='opdrachtknoppen'>\n");
	if ($currenttab != "Zelfservice" and ($i_lid->soortlid == "Lid" or $i_lid->soortlid == "Toekomstig lid") and $auc > 1) {
	...
	*/
	...
	if ($p_type == "A" or $p_type == "BCF") {
		...
		if (count($lorows) > 0 and max(array_column($lorows, "Functie")) > 0) {
			$kols[] = array('columnname' => "EmailFunctie", 'headertext' => "E-mail bij functie", 'type' => "email", 'cond_ro' => "RO_Email", 'class' => "w25");
		}
	...
	if (count($lorows) > 0 and max(array_column($lorows, "MogelijkeParttimeperc")) > "") {
		$kols[] = array('columnname' => "Parttimepercentage", 'headertext' => "Partime", 'bronselect' => "MogelijkeParttimeperc", 'class' => "w10");
	}
	
	if ($p_type != "O") {
		$kols[] = array('columnname' => "Opmerk", 'headertext' => "Opmerking", 'class' => "w25");
...
//				debug($row->Naam . ": " . $_POST[$row->Naam]);
	...
	printf("<label class='form-label'>Leden, onderdelen per lid en lidmaatschappen</label><input type='number' class='num3' value=%d><p>maanden na einde laatste lidmaatschap</p>\n", $_SESSION['settings']['ledenopschonen']);
	printf("<label class='form-label'>Lid definitief verwijderen</label><input type='number' class='num3' name='liddefinitiefverwijderen' value=%d OnChange='this.form.submit();'><p>aantal maanden na verwijdermarkering</p>\n", $_SESSION['settings']['liddefinitiefverwijderen']);
	printf("<label class='form-label'>Diploma's van lid</label><input type='number' class='num3' name='liddipl_bewaartermijn' value=%d OnChange='this.form.submit();'><p>aantal maanden na einde lidmaatschap</p>\n", $_SESSION['settings']['liddipl_bewaartermijn']);
	printf("<label class='form-label'>Aan een lid gekoppelde logging</label><input type='number' class='num3' name='logboek_lid_bewaartijd' value=%d OnChange='this.form.submit();'><p>aantal maanden</p>\n", $_SESSION['settings']['logboek_lid_bewaartijd']);
	printf("<label class='form-label'>Foto's van leden</label><input type='number' class='num3' name='fotosledenverwijderen' value=%d OnChange='this.form.submit();'><p>aantal maanden na beëindiging lidmaatschap</p>\n", $_SESSION['settings']['fotosledenverwijderen']);
	echo("</form>\n");
	...
	for ($sw=$dtStart;$sw <= strtotime("+29 day");$sw=strtotime("+1 day", $sw)) {
		...
		if ($sw < strtotime("+14 day")) {
			...
			foreach ($i_ak->lijst(-1, "", $f, "AK.Datum", 3) as $row) {
				...
				$i_lo->vulvars(-1, $_SESSION['lidid'], $row->OnderdeelID, $hd);

				if (strlen($i_lo->i_gr->starttijd) > 3 and $row->Activiteit == 1) {
...
function fnBasisgegevens($p_type) {
	...
	if ($p_type == "Onderdelen") {
		...
		foreach (ARRTYPEONDERDEEL as $k => $v) {
			...
			if ($i_ond->aantal() > 0) {
				$opt .= sprintf("<option value='%s'%s>%s</option>\n", $k, checked($filtertypeonderdeel, "option", $k), $v);
			}
		}
		$i_ond->where = "";
		...
		printf("<button type='submit' class='%s btn-sm' name='NieuwOnderdeel' onClick='this.form.submit();'>%s Onderdeel</button>\n", CLASSBUTTON, ICONTOEVOEGEN);
		...
		echo(fnEditTable($ondrows, $kols, "onderdeeledit", "Onderdelen muteren"));
	...
	} elseif ($p_type == "Commissies") {
		onderdelenmuteren("C");
		
	} elseif ($p_type == "Selecties") {
		onderdelenmuteren("S");
		...
		if ((new cls_Onderdeel())->aantal("O.`Type`='A'") > 0) {
			$kols[] = array('headertext' => "Bij afdeling?", 'columnname' => "Afdelingsfunctie", 'type' => "checkbox");
		}
		$kols[] = array('headertext' => "Algemeen?", 'columnname' => "Ledenadministratiefunctie", 'type' => "checkbox");
		$kols[] = array('headertext' => "Kader?", 'columnname' => "Kader", 'type' => "checkbox");
		$kols[] = array('headertext' => "Inval?", 'columnname' => "Inval", 'type' => "checkbox");
		$kols[] = array('headertext' => "Vervallen per", 'columnname' => "Vervallen per", 'type' => "date");
		...
		$kols[] = array('headertext' >= "Nummer", 'columnname' => "Nummer", 'type' => "pk", 'cond_ro' => "aantalRek");
		$kols[] = array('headertext' => "Begindatum", 'columnname' => "Begindatum", 'type' => "rqdate");
		$kols[] = array('headertext' => "Einddatum", 'columnname' => "Einddatum", 'type' => "rqdate");
		$kols[] = array('headertext' => "Jeugdlid tot", 'columnname' => "Leeftijdsgrens jeugdleden", 'type' => "leeftijd");
		...
		$kols[] = array('headertext' => "&nbsp;", 'columnname' => "Nummer", 'class' => "trash", 'link' => $l, 'cond_ro' => "aantalRek");
		...
		echo(fnEditTable($seizres, $kols, "seizoenedit", "Seizoenen muteren"));
	...
	if ($ondid > 0 or $i_ev->evid > 0) {
		...
		foreach ($rows as $row) {
			...
			if ($eventid > 0) {
				...
				if (strlen($i_ed->statusoms) > 0 and $i_ed->aanwezig == false) {
					$stat = ": " . $i_ed->statusoms;
				}
			}
