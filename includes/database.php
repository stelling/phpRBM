...
class cls_db_base {
	...
	public string $refcolumn = "";		// Naam van de kolom bij een update
	public string $typecolumn = "";		// Type van de kolom
	...
	public $selectgroep = "CASE 
					...
					ELSE LO.GroepID
					END";
	public string $selectgebdatum = "";
	public string $selectmhaan = "";
	
	const SELECTNAAM = "CONCAT(IF(LENGTH(IFNULL(L.Roepnaam, ''))>1, TRIM(L.Roepnaam), IFNULL(L.Voorletter, '')), ' ', IF(IFNULL(L.Tussenv, '')>'', CONCAT(L.Tussenv, ' '), ''), L.Achternaam)";
	...
	function __construct($p_table="") {
		...
		$this->selectmhaan = sprintf("CONCAT(IF(MH.LidID=0, '', CONCAT(%s, ' & ')), MH.to_addr, IF(LENGTH(IFNULL(MH.cc_addr, '')) < 5, '', CONCAT(' & CC: ', MH.cc_addr)))", self::SELECTNAAM);
		$this->per = date("Y-m-d");
	}
...
//					debug($this->query);
					...
					$ts = $_SESSION['settings']['performance_trage_select'] ?? 0;
					if ($ts > 0 and $exec_tijd >= $ts) {
...
//							$mess = sprintf("Uitgevoerd zonder resultaat: %s", $this->query);
	...
	}  # cls_db_base->is_kolom_numeriek
	
	public function laatstemutatie($p_rid, $p_tas=-1, $p_glue="<br>") {
		global $dtfmt;
		
		$rv = "";
		$dtfmt->setPattern(DTLONG);
		
		$query = sprintf("SELECT AA.LidID, AA.DatumTijd, %1\$s AS NaamLid FROM %2\$sAdmin_activiteit AS AA INNER JOIN %2\$sLid AS L ON AA.LidID=L.RecordID", self::SELECTNAAM, TABLE_PREFIX);
		$query .= sprintf(" WHERE AA.LidID > 0 AND AA.RefTable='%s' AND ReferID=%d", str_replace(TABLE_PREFIX, "", $this->table), $p_rid);
		if ($p_tas > 0) {
			$query .= sprintf(" AND AA.TypeActiviteitSpecifiek=%d", $p_tas);
		}
		$query .= " ORDER BY AA.DatumTijd DESC LIMIT 1;";

		$lbrow = $this->execsql($query)->fetch();
		if (isset($lbrow->DatumTijd)) {
			$rv = sprintf("%s%s%s", $lbrow->NaamLid, $p_glue, $dtfmt->format(strtotime($lbrow->DatumTijd)));
		} elseif (($p_tas == 1 or $p_tas == 11) and $this->bestaat_kolom("Ingevoerd")) {
			$dtfmt->setPattern(DTTEXT);
			$query = sprintf("SELECT IFNULL(MAX(Ingevoerd), '') FROM %s WHERE %s=%d", $this->table, $this->pkkol, $p_rid);
			$rv = $dtfmt->format(strtotime($this->scalar($query)));
		} elseif ($this->bestaat_kolom("Gewijzigd")) {
			$query = sprintf("SELECT IFNULL(MAX(Gewijzigd), '') FROM %s WHERE %s=%d", $this->table, $this->pkkol, $p_rid);
			$rv = $dtfmt->format(strtotime($this->scalar($query)));
		}

		return $rv;
}  # cls_db_base->laatstemutatie
	...
	public function nieuwrecordid($p_min=1) {
		...
		*/
		...
		if ($tabel == "Organisatie") {
			...
			if ($r > $rid) {$rid = $r; }
			$r = $this->scalar(sprintf("SELECT IFNULL(MAX(Ord.Ordernr), 0) FROM %sWS_Orderregel AS Ord;", TABLE_PREFIX));
			if ($r > $rid) {$rid = $r; }
...
//		debug("max function: " . $p_kolom . " / " . $tk);
		
		if ($tk == "date" or $tk == "datetime" or $tk == "function") {
...
//		debug($mess, 1, 1);
		...
		if ($p_recid >= 0 and strlen($p_kolom) > 0 and strlen($this->table) > 0) {
			...
			if (($tk == "date" or $tk == "datetime") and (strlen($p_waarde) < 8 or $p_waarde <= "1900-01-01")) {
				...
				$p_waarde = "NULL";
			} elseif ($this->is_kolom_numeriek("", $tk) and ($p_waarde == null or strlen(trim($p_waarde)) == 0)) {
				$p_waarde = 0;
			...
			if ($p_waarde === "NULL") {
				...
				$xw = sprintf("BINARY (IFNULL(`%1\$s`, '')<>:nw)", $p_kolom);
...
class cls_Lid extends cls_db_base {
	...
	public string $avgnaam = "";
	public string $meisjesnaam = "";		// Deze is vervallen, maar er kan nog een waarde in zitten
	public string $geslacht = "O";
	public string $geboortedatum = "";
	public string $geboorteplaats = "";
	public string $geboren = "";			// Combinatie van geboortedatum in tekst en geboorteplaats
	public string $overleden = "";
	public string $adres = "";
	public string $postcode = "";
	public string $woonplaats = "";
	public int $buitenland = 0;
	public string $telefoon = "";
	public string $mobiel = "";
	public string $email = "";
	public string $emailvereniging = "";
	public string $emailouders = "";
	public string $namenouders = "";
	public string $waarschuwenbijnood = "";
	public int $huisnr = 0;
	public string $huisletter = "";
	public string $toevoeging = "";
	public int $rekeningbetaalddoor = 0;
	public string $bankrekening = "";
	public string $sportlinkid = "";
	public string $vogafgegeven = "";
	...
	public float $lengtelidmaatschap = -1;
	...
	public function vulvars($p_lidid=-1, $p_per="") {
		...
		if ($this->lidid > 0) {
			...
			if (isset($row->RecordID) and $row->RecordID > 0) {
				...
				$this->buitenland = $row->Buitenland ?? 0;
				$this->mobiel = $row->Mobiel ?? "";
				
				if (strlen($this->mobiel) >= 10) {
					$this->telefoon = $this->mobiel;
				} else {
		...
		if ($this->lidid > 0) {
			...
			if (strlen($this->roepnaam) > 1) {
				$this->avgnaam = trim($this->roepnaam);
			} else {
				$this->avgnaam = $this->voorletters;
				if (substr($this->avgnaam, -1, 1) != ".") {
					$this->avgnaam .= ".";
				}
			}
			$tv = trim($this->tussenvoegsels ?? "");
			if (strlen($tv) > 0)  {
				$tv = str_replace("van der", "v/d", $tv);
				$tv = str_replace("van den", "v/d", $tv);
				$tv = str_replace("van de", "v/d", $tv);
				$this->avgnaam .= " " . $tv;
			}
			$this->avgnaam .= " " . substr($this->achternaam, 0, 1) . ".";					
			
			if ($this->geboortedatum > "1900-01-01" and strlen($this->overleden) == 0) {
				$this->leeftijd = date("Y", strtotime($this->per)) - date("Y", strtotime($this->geboortedatum));
				if (date("m-d", strtotime($this->per)) < date("m-d", strtotime($this->geboortedatum))) {
					$this->leeftijd--;
				}
			}
			$dtfmt->setPattern(DTTEXT);
			$this->geboren = $dtfmt->format(strtotime($this->geboortedatum));
			if (strlen($this->geboorteplaats) > 0) {
				$this->geboren .= " te " . $this->geboorteplaats;
			}
			
			(new cls_Parameter())->vulsessie();
			
			if (in_array($this->lidid, $lididwebmasters)) {
				$this->inloggentoegestaan = true;
			} elseif (strlen($_SESSION['settings']['login_beperkttotgroep']) > 0) {
				$query = sprintf("SELECT COUNT(*) FROM %sLidond AS LO WHERE LO.Vanaf <= CURDATE() AND IFNULL(LO.Opgezegd, '9999-12-31') >= CURDATE() AND LO.Lid=%d AND LO.OnderdeelID IN (%s);", TABLE_PREFIX, $this->lidid, $_SESSION['settings']['login_beperkttotgroep']);
				if ($this->scalar($query) > 0) {
					$this->inloggentoegestaan = true;
				} else {
				...
				$query = sprintf("SELECT COUNT(*) FROM %sLidmaatschap AS LM WHERE LM.LIDDATUM <= CURDATE() AND IFNULL(LM.Opgezegd, '9999-12-31') >= CURDATE() AND LM.Lid=%d;", TABLE_PREFIX, $this->lidid);
				if ($this->scalar($query) > 0) {
					$this->inloggentoegestaan = true;
				} else {
					$this->inloggentoegestaan = false;
	...
	public function record($p_lidid=-1) {
		...
		if ($this->lidid > 0) {
			$query = sprintf("SELECT L.*, %s as NaamLid FROM %s WHERE L.RecordID=%d;", self::SELECTNAAM, $this->basefrom, $this->lidid);
			$result = $this->execsql($query);
			$row = $result->fetch();
	...
	public function ledenlijst($p_soortlid=1, $p_ondfilter=-1, $p_ord="", $p_filter="", $p_metadres=0) {	
		...
		*/
		...
		if ($p_soortlid == 1) {
			...
			$filter .= sprintf(" AND IFNULL(LM.Opgezegd, '9999-12-31') < '%s'", $this->per);
		} elseif ($p_soortlid == 4) {
		...
		$query = sprintf("SELECT DISTINCT L.RecordID, %s AS `NaamLid`%s, L.GEBDATUM, %s AS Zoeknaam, L.Postcode, L.RekeningBetaaldDoor, L.Achternaam, L.TUSSENV, L.Roepnaam, L.RecordID AS LidID, L.Opmerking
					...
					%s
					GROUP BY L.RecordID
					ORDER BY %sL.Achternaam, L.TUSSENV, L.Roepnaam, LM.LIDDATUM;", self::SELECTNAAM, $xtraselect, $this->selectzoeknaam, $from, $filter, $p_ord);
		$result = $this->execsql($query);
		return $result->fetchAll();
	...
	public function jubilarissen($p_per) {
		...
		$query = sprintf("SELECT L.RecordID, %1\$s AS Naam, (%4\$s) AS LengteLidmaatschap, (%5\$s) AS LengteKaderlidmaatschap, (%6\$s) AS EindeKaderlidmaatschap
								...
								ORDER BY L.Achternaam, L.TUSSENV, L.Roepnaam;", self::SELECTNAAM, $this->fromlid, $p_per, $sqlm, $sqkad, $sqek);
		$result = $this->execsql($query);
	...
	public function overzichtlid($p_lidid) {
		...
		$query = sprintf("SELECT 'Persoonsgegevens' AS Kop1, 
					...
					WHERE L.RecordID=%d 
					ORDER BY LM.LIDDATUM DESC;", self::SELECTNAAM, $this->selectgeslacht, $this->selectgebdatum, $this->selectleeftijd, $adresgegevens, $sqond, $sqlll, $sqllegitimatie, $sqllogin, $VOG, $this->table, TABLE_PREFIX, $p_lidid);
		$result = $this->execsql($query);
		return $result->fetch();
	...
	public function lijstselect($p_filter=0, $p_xf="", $p_per="", $p_ondid=-1) {
		...
		$query = sprintf("SELECT DISTINCT L.RecordID, %s AS NaamLid, %s AS Zoeknaam ", self::SELECTNAAM, $this->selectzoeknaam);
		if ($p_filter == 1) {
			// Alleen leden
			...
			// Alleen personen die iets kunnen bestellen
			$query .= sprintf("FROM %1\$s INNER JOIN %2\$sLidond AS LO ON L.RecordID=LO.Lid WHERE IFNULL(LO.Opgezegd, '9999-12-31') >= '%3\$s' AND LO.OnderdeelID IN (SELECT BeperkTotGroep FROM %2\$sWS_Artikel AS Art WHERE IFNULL(Art.VervallenPer, '9999-12-31') > '%3\$s')", $this->basefrom, TABLE_PREFIX, $this->per);
	...
	public function htmloptions($p_cv=-1, $p_filter=0, $p_xf="", $p_per="", $p_ondid=-1) {
		...
		foreach($this->lijstselect($p_filter, $p_xf, $p_per, $p_ondid) as $lrow) {
			$this->vulvars($lrow->RecordID, $p_per);
			if ($lrow->RecordID == $p_cv) {
				$s = " selected";
			} else {
				$s = "";
			}
			...
			if ($p_filter == 0 or $p_filter == 2) {
				if ($this->soortlid != "Lid") {
					$tw .= sprintf(" (%s)", substr($this->soortlid, 0, 1));
				}
			}
	...
	public function lidbijemail($p_email, $p_xf="") {
		...
		$w = sprintf("IFNULL(LM.Opgezegd, '9999-12-31') >= '%1\$s' AND (LOWER(L.Email)='%2\$s' OR LOWER(L.EmailOuders)='%2\$s' OR LOWER(L.EmailVereniging)='%2\$s')", $this->per, $p_email);
		
		$query = sprintf("SELECT L.RecordID AS LidID, LM.Lidnr FROM %s LEFT OUTER JOIN %sLidmaatschap AS LM ON L.RecordID=LM.Lid WHERE %s;", $this->basefrom, TABLE_PREFIX, $w);
		$rows = $this->execsql($query)->fetchAll();
		if (count($rows) == 1) {
			$this->vulvars($rows[0]->LidID);
		} elseif (count($rows) == 0) {
			$this->lidid = 0;
		} elseif (strlen($p_xf) > 0) {
			$w .= " AND " . $p_xf;
	...
	public function add($p_achternaam, $p_postcode="") {
		...
		if ($dbc->prepare($query)->execute($data) > 0) {
			...
			$this->mess = sprintf("Tabel Lid: record %d (%s) is toegevoegd", $this->lidid, $p_achternaam);
		} else {
			$this->mess = sprintf("Tabel Lid: toevoegen record voor %s is mislukt", $p_achternaam);
			$this->lidid = 0;
		}
		$this->log($this->lidid);
		
		if (strlen($p_postcode) >= 6 and $this->lidid > 0) {
...
class cls_Lidmaatschap extends cls_db_base {
	...
	public int $lidnr = 0;
	public string $lidvanaf = "";
	public string $opgezegd = "";
	public int $opgezegddoorvereniging = 0;
	
	public string $lidtm = "";
	public string $opgezegdper = "";	// De datum per wanneer iemand geen lid meer is in de Nederlandse taal
	...
	public function vulvars($p_lmid, $p_lidid=-1, $p_lidnr=-1) {
		...
		if ($this->lmid > 0) {
			...
			if (isset($row->RecordID)) {
				...
				$this->opgezegddoorvereniging = $row->OpgezegdDoorVereniging ?? 0;
		...
		$this->i_lid = new cls_Lid($this->lidid, $this->per);
		
		if ($this->opgezegd > "1970-01-01") {
			$dtfmt->setPattern(DTTEXT);
	...
	public function lidid($p_lmid, $p_lidnr=-1) {
	...
	public function islid($p_lidid=-1, $p_per="") {
		...
		return $this->i_lid->islid;
		
	}  # cls_Lidmaatschap->islid
	
	public function soortlid($p_lidid=-1, $p_per="") {
		
		debug(__CLAS__ . "->" . __FUNCTION__ . ": Beter vermijden", 0, 99);
	...
	public function add($p_lidid, $p_vanaf="") {
		...
		$f = sprintf("LM.Lid=%d", $p_lidid);
		if ($this->aantal($f . " AND (LM.Opgezegd IS NULL)") > 0) {
			$this->mess = "Er is nog een lopend lidmaatschap voor dit lid, dit lidmaatschap wordt niet toegevoegd.";
			$nrid = 0;
		} else {
			$nrid = $this->nieuwrecordid();
			$query = sprintf("INSERT INTO %s (RecordID, Lid, Lidnr, LIDDATUM) VALUES (%d, %d, %d, '%s');", $this->table, $nrid, $this->lidid, $lidnr, $p_vanaf);
			if ($this->execsql($query) > 0) {
				$this->mess = sprintf("Tabel Lidmaatschap: Record %d met lidnummer %d en ingangsdatum %s is toegevoegd.", $nrid, $lidnr, $p_vanaf);
			} else {
				$this->mess = "Tabel Lidmaatschap: toevoegen record is mislukt.";
				$nrid = 0;
			}
		}
		$this->Log($nrid);
	...
	public function update($p_lmid, $p_kolom, $p_waarde, $p_reden="") {
		$this->vulvars($p_lmid);
		$this->tm = 0;
		...
		if (strlen($p_waarde) < 5 and $p_kolom == "LIDDATUM") {
			...
			$this->pdoupdate($p_lmid, $p_kolom, $p_waarde, $p_reden);
		}
		$this->log($p_lmid);
	}
	...
	public function controle($p_lidid=-1) {
		...
		$this->where = "IFNULL(LM.Opgezegd, '9999-12-31') > CURDATE()";
		if ($p_lidid > 0) {
			$this->where = sprintf("LM.Lid=%d", $p_lidid);
		}
		
		foreach ($this->basislijst() as $lmrow) {
			$this->vulvars($lmrow->RecordID);
			if (strlen($this->i_lid->overleden) == 10 and $this->i_lid->overleden < date("Y-m-d", strtotime("-7 day")) and $this->lidtm > $this->i_lid->overleden) {
				$this->update($lmrow->RecordID, "Opgezegd", $this->i_lid->overleden, "het lid overleden is");
			}
	...
	public function opschonen($p_lidid=-1) {
		...
		if ($p_lidid <= 0) {
			$query = sprintf("SELECT LM.RecordID, LM.Lid FROM %s WHERE (LM.Lid NOT IN (SELECT L.RecordID FROM %sLid AS L WHERE (L.Verwijderd IS NULL)));", $this->basefrom, TABLE_PREFIX);
			$result = $this->execsql($query);
			foreach ($result->fetchAll() as $row) {
				$this->lidid = $row->Lid;
				$this->pdodelete($row->RecordID, "het Lid niet bestaat");
				$this->Log($row->RecordID);
			}
		}
		
		$this->where = sprintf("LM.LIDDATUM > IFNULL(LM.Opgezegd, '9999-12-31')");
		if ($p_lidid > 0) {
			$this->where .= sprintf(" AND LM.Lid=%d", $p_lidid);
		}
...
class cls_Authorisation extends cls_db_base {
	...
	public int $aid = 0;
	public string $naamtp = "";
	public int $ondid = 0;
	public string $ondnaam = "";
	public $toegang = -1;
	public string $laatstgebruikt = "";
	...
	public function vulvars($p_aid=-1, $p_tabpage="") {
		$this->aid = $p_aid;
		
		if ($this->aid <= 0 and strlen($p_tabpage) > 0) {
			$query = sprintf("SELECT AA.RecordID FROM %s WHERE LOWER(AA.Tabpage)='%s';", $this->basefrom, strtolower($p_tabpage));
			$rows = $this->execsql($query)->fetchAll();
			if (count($rows) == 1) {
		...
		if ($this->aid > 0) {	
			...
			$this->laatstgebruikt = $row->LaatstGebruikt ?? "";
			$this->ingevoerd = $row->Ingevoerd ?? "";
			$this->gewijzigd = $row->Gewijzigd ?? "";
		}
		if ($this->toegang == -2) {
			$this->ondnaam = "Niemand";
		} elseif ($this->toegang == -1) {
			$this->ondnaam = "Alleen webmasters";
		} elseif ($this->toegang == 0) {
			$this->ondnaam = "Iedereen";
		} else {
			$this->ondid = $this->toegang;
			$this->ondnaam = (new cls_Onderdeel())->Naam($this->toegang);
		}
	...
	public function recordid($p_tp) {
		...
		$tp = str_replace("'", "", $p_tp);
		$query = sprintf("SELECT IFNULL(MAX(RecordID), 0) FROM %s WHERE LOWER(Tabpage)=\"%s\"", $this->table, str_replace("'", "", strtolower($tp)));
		$this->aid = $this->scalar($query);
		$this->vulvars($this->aid);
		return $this->aid;
	}
...
class cls_Onderdeel extends cls_db_base {
	...
	function __construct($p_oid=-1, $p_type="") {
		...
		$this->per = date("Y-m-d");
		$this->vulvars($p_oid, $p_type);
		foreach (ARRTYPEONDERDEEL as $k => $v) {
			$this->mogelijketypes .= $k;
		}
	}

	public function vulvars($p_oid, $p_type="") {
		$this->ta = 20;
		if ($p_oid >= 0) {
			$this->oid = $p_oid;
		}
		...
		if ($this->oid > 0) {
			...
			if (isset($row->RecordID)) {
				...
				$this->bewaartermijnpresentie = $row->BewaartermijnPresentie ?? 0;
				...
				$this->contributieperactiviteit = $row->ContributiePerActiviteit ?? 0;
				$this->mogelijkeparttimeperc = $row->MogelijkeParttimeperc ?? "";
		...
		if (strlen($this->type) > 0 and array_key_exists($this->type, ARRTYPEONDERDEEL)) {
	...
	public function record($p_oid=-1, $p_kode="") {
		...
		debug(__CLASS__ . "->" . __FUNCTION__ . ": Beter vermijden", 0, 99);
	...
	public function lijst($p_ingebruik=1, $p_filter="", $p_per="", $p_lidid=0, $p_orderby="", $p_fetched=1) {
		...
		$sqal = sprintf("SELECT COUNT(DISTINCT LO.Lid) FROM %sLidond AS LO WHERE %s AND LO.OnderdeelID=O.RecordID", TABLE_PREFIX, str_replace("CURDATE()", "'" . $p_per . "'", cls_db_base::$wherelidond));
...
//		debug($query);
	...
	public function htmloptions($p_cv=-1, $p_ingebruik=1, $p_ondtype="", $p_aant=1) {
		...
		*/
		...
		if ($p_ingebruik == 1) {
			...
			$f .= sprintf(" (SELECT COUNT(*) FROM %sLidond AS LO WHERE %s AND LO.OnderdeelID=O.RecordID) > 0", TABLE_PREFIX, str_replace("CURDATE()", "'" . $this->per . "'", cls_db_base::$wherelidond), $p_cv);
		}
		
		if (strlen($f) > 0 and $p_cv > 0) {
			$f = "((" . $f . sprintf(") OR O.RecordID=%d)", $p_cv);
		}
	...
	public function update($p_oid, $p_kolom, $p_waarde, $p_reden="") {
		...
		} elseif ($p_kolom == "MogelijkeParttimeperc" and strlen($p_waarde) > 0) {
			$p_waarde = str_replace(".", ",", $p_waarde);
			$p_waarde = str_replace(",00;", ";", $p_waarde);
			$p_waarde = str_replace(",0;", ";", $p_waarde);
			if (strlen(strpos($p_waarde, "100") === false)) {
				$p_waarde .= ";100";
			} elseif (strlen($p_waarde) == 3) {
				$p_waarde = "";
			}
	...
	public function controle() {		
		...
		foreach ($this->basislijst() as $row) {
			...
			} elseif ($this->vervallen > "2000-01-01" and $this->vervallen < date("Y-m-d") and strlen($this->email) > 0) {
				$this->update($row->RecordID, "CentraalEmail", "", "het onderdeel vervallen is.");
			} elseif (strlen($this->mysql) > 0 and (strlen($this->mysql) <= 10 or substr($this->mysql, 0, 6) != "SELECT")) {
				$this->update($row->RecordID, "MySQL", "", "de mySQL-code niet correct is");
			} elseif (strpos("EMOT", $row->Type) !== false and $row->Kader == 1) {
				$reden = "bij een eigenschap, materiaalbeheer, onderscheiding of toestemming mag kader geen ja zijn";
				$this->update($row->RecordID, "Kader", 0, $reden);
			} elseif (strlen($this->mysql) > 0 and $this->alleenleden > 0) {
	...
	public function opschonen() {
		$this->where = sprintf("WHERE IFNULL(O.VervallenPer, '9999-12-31') < CURDATE() AND IFNULL(O.Ingevoerd, CURDATE()) < DATE_SUB(CURDATE(), INTERVAL %d DAY) AND (O.RecordID NOT IN (SELECT LO.OnderdeelID FROM %sLidond AS LO));", BEWAARTIJDNIEUWERECORDS, TABLE_PREFIX);
...
class cls_Functie extends cls_db_base {
	...
	private function vulvars($p_fnkid=-1) {
		...
		$this->naam = "";
		if ($this->fnkid > 0) {
			$query = sprintf("SELECT * FROM %s WHERE F.Nummer=%d;", $this->basefrom, $this->fnkid);
			$row = $this->execsql($query)->fetch();
			if (isset($row->Nummer)) {
				$this->naam = $row->Omschrijv ?? "";
				$this->afkorting = $row->Afkorting ?? "";
				$this->kader = $row->Kader ?? 0;
				$this->ingevoerd = $row->Ingevoerd ?? "";
				$this->gewijzigd = $row->Gewijzigd ?? "";
			} else {
				$this->fkid = 0;
...
class cls_Afdelingskalender extends cls_db_base {
	...
	public function vulvars($p_akid=-1, $p_ondid=-1) {
		...
		if ($this->akid > 0) {
			...
			if (isset($row->RecordID)) {
				...
				$this->omschrijving = str_replace("\"", "'", trim($row->Omschrijving ?? ""));
	...
	public function htmloptions($p_onderdeelid, $p_cv, $p_filter="") {
		...
		foreach ($this->lijst($p_onderdeelid, "", $p_filter) as $row) {
			...
			if (strlen($this->omschrijving) > 0) {
				$o .= " " . substr($this->omschrijving, 0, 35);
			}
...
class cls_Aanwezigheid extends cls_db_base {
	...
	public function vulvars($p_loid, $p_akid=-1) {
		...
		if ($this->loid > 0 and $this->akid > 0) {
			...
			if (isset($awrow->RecordID)) {
				...
				$this->ingevoerd = $awrow->Ingevoerd ?? "";
			}
		}
		...
		if (strlen($this->status) > 0 and array_key_exists($this->status, ARRPRESENTIESTATUS)) {
			$this->statusoms = ARRPRESENTIESTATUS[$this->status];
		} else {
			$this->statusoms = "";
		}
	...
	private function add($p_loid, $p_akid, $p_status="") {
		...
		$this->aanwid = $nrid;
		$this->log($nrid, 0, $this->i_lo->ondid);
	...
	public function update($p_loid, $p_akid, $p_kolom, $p_waarde) {
		...
		$s = "";
		if ($p_kolom == "Status" and array_key_exists($p_waarde, ARRPRESENTIESTATUS)) {
			$s = $p_waarde;
		}
		...
		if ($this->aanwid > 0 and $this->pdoupdate($this->aanwid, $p_kolom, $p_waarde) > 0) {
			$this->log($this->aanwid, 0, $this->i_lo->ondid);
		}
	}
...
class cls_Lidond extends cls_db_base {
	...
	public function vulvars($p_loid=-1, $p_lidid=-1, $p_ondid=-1, $p_per="", $p_incldipl=0) {
		
		/*
			p_incldipl = de voortgang bij diploma's worden aan de class en titel toegevoegd
		*/
		...
		if ($this->loid > 0) {
			...
			if (isset($row->RecordID)) {
				...
				$this->opmerking = $row->Opmerk ?? "";
				$this->parttimepercentage = $row->Parttimepercentage ?? 100;
				$this->functieid = $row->Functie ?? 0;
		...
		$this->i_func = new cls_Functie($this->functieid);
		$i_ak->where = sprintf("AK.OnderdeelID=%d AND AK.Activiteit=1", $this->ondid);
		
		if ($_SERVER['PHP_SELF'] == "/maatwerk/jobs.php" or WEBMASTER) {
		...
		$this->kader = $this->i_ond->iskader;
		if ($this->i_func->kader == 1) {
			$this->kader = true;
		}
		...
		if ($this->laatstemutatiegroep > date("Y-m-d", strtotime("-4 week"))) {
			$this->loclass .= " gewijzigd";
		}

		if ($this->groepid > 0 and $this->i_gr->dpid > 0 and $p_incldipl == 1) {
			...
			if ($this->i_gr->i_dp->voorgangerid > 0) {
				$f = sprintf("LD.Lid=%d AND LD.DiplomaID=%d AND LD.DatumBehaald <= '%s' AND LD.Geslaagd=1 AND LD.LaatsteBeoordeling=1", $this->lidid, $this->i_gr->i_dp->voorgangerid, $this->per);
				...
				if (strlen($dvd) < 10) {
					...
					$this->lotitle .= sprintf("%s ontbreekt", $this->i_gr->i_dp->naamvoorganger);
				}
			} else {
				$f = sprintf("LD.Lid=%d AND LD.DatumBehaald <= '%s' AND LD.Geslaagd=1 AND LD.LaatsteBeoordeling=1", $this->lidid, $this->per);
				$dvd = $i_ld->max("DatumBehaald", $f);
			}
			
			if (strlen($dvd) == 10) {
				$startdatumvoortgang = $dvd;
			}

			$f = sprintf("AK.Datum > '%s' AND AK.Datum < CURDATE() AND AK.Activiteit=1", $startdatumvoortgang);
			$this->werkelijkedoorlooptijd = $i_ak->aantal($f);
				
			if ($this->i_gr->i_dp->doorlooptijd > 0 and $this->werkelijkedoorlooptijd > $this->i_gr->i_dp->doorlooptijd) {
			...
			if ($this->i_gr->i_dp->voorgangerid > 0) {
				...
				if (strlen($dvd) == 10) {
					$this->lotitle .= sprintf("%s behaald op %s", $this->i_gr->i_dp->naamvoorganger, date("d-m-Y", strtotime($dvd)));
				} else {
					$this->lotitle .= sprintf("lid is gestart op %s", date("d-m-Y", strtotime($startdatumvoortgang)));
				}
				if ($this->werkelijkedoorlooptijd > 1) {
					$this->lotitle .= sprintf(" (%d lessen)", $this->werkelijkedoorlooptijd);
				}
			}
	...
	public function lijst($p_ondid, $p_filter=1, $p_ord="GR.Volgnummer, GR.Kode", $p_limiet=0, $p_fetched=1) {
		...
		if (toegang("Woonadres_tonen", 0, 0)) {
			$seladres = ", L.Adres, L.Postcode, L.Woonplaats";
		}
		...
		$query = sprintf("SELECT LO.RecordID, LO.Lid AS LidID, LO.OnderdeelID, LO.Opmerk, LO.Vanaf, LO.Opgezegd, LO.EmailFunctie, LO.GroepID, LO.Functie, LO.Lid, LO.Parttimepercentage,
								%s AS NaamLid, L.Roepnaam, L.Achternaam, L.Tussenv, %s AS AVGnaam, L.GEBDATUM, %s AS Leeftijd%s, %s AS Telefoon, %s AS Email, L.EmailVereniging, L.RelnrRedNed, 
								F.Omschrijv AS FunctieOms, F.Afkorting AS FunctAfk, F.Inval AS Invalfunctie, %s AS Groep, GR.DiplomaID,
								O.Kode, O.Naam AS OndNaam, O.CentraalEmail, IF(LO.Functie > 0, 0, 1) AS RO_Email, O.MogelijkeParttimeperc,
								GR.Kode AS GrCode, GR.Omschrijving AS GrNaam, GR.Starttijd, GR.Aanwezigheidsnorm, IFNULL(Act.BeperkingAantal, 0) AS BeperkingAantal, IFNULL(Act.Omschrijving, '') AS OmschrijvingActiviteit,
								CASE WHEN LO.GroepID > 0 AND LO.Functie > 0 THEN CONCAT(F.OMSCHRIJV, '/', IF(LENGTH(GR.Kode)=0, GR.RecordID, GR.Kode))
									...
									WHEN LO.GroepID > 0 THEN CONCAT(TRIM(GR.Starttijd), ' ', IF(LENGTH(GR.Omschrijving)=0, IF(LENGTH(GR.Kode)=0, GR.RecordID, GR.Kode), GR.Omschrijving), IF(LO.parttimepercentage > 1 AND LO.parttimepercentage < 100, CONCAT(' (', LO.Parttimepercentage, '%%)'), ''))
	...
	public function groepsindeling($p_onderdeelid, $p_filter="", $p_inclkader=0) {
		...
		$this->query = sprintf("SELECT DISTINCT O.Naam AS AfdNaam, CONCAT(GR.Starttijd, IF(LENGTH(GR.Eindtijd) > 3, CONCAT(' - ', GR.Eindtijd), '')) AS Tijdsblok, GR.Kode, GR.Omschrijving,
					...
					FROM %3\$s
					WHERE %4\$s
					ORDER BY O.Naam, IF(LO.GroepID=0 OR LENGTH(TRIM(IFNULL(GR.Starttijd, '')))=0, '99:99', GR.Starttijd), GR.Volgnummer, GR.Omschrijving, GR.RecordID, L.Achternaam, L.Roepnaam;", $this->selectnaam, $this->selectleeftijd, $this->fromlidond, $where, $this->selectgroep, $this->selectavgnaam);
		$result = $this->execsql();
		return $result->fetchAll();
	...
	public function laatstemutatiegroep($p_loid=-1) {
	...
	public function islid($p_lidid=-1, $p_ondid=-1, $p_per="") {
		$this->vulvars(0, $p_lidid, $p_ondid, $p_per);
	...
	public function add($p_ondid, $p_lidid, $p_reden="", $p_vanaf="", $p_toonlog=0) {
		...
		$i_lm = new cls_Lidmaatschap(-1, $p_lidid);
		...
		if (strlen($p_vanaf) < 8) {
			...
			} elseif ($this->i_ond->alleenleden == 1 and $i_lm->lidvanaf > date("Y-m-d")) {
				$vanaf = $i_lm->lidvanaf;
			} elseif ($this->i_ond->alleenleden == 1 and (new cls_lid($p_lidid))->soortlid == "Voormalig lid") {
				$vanaf = $i_lm->lidvanaf;
		...
		if (strlen($this->i_ond->naam) == 0) {
			...
			$this->mess = sprintf("Dit record wordt niet toegevoegd, want de combinatie van %s, %s en vanaf (%s) al in de tabel staat.", $this->i_lid->naam, $this->i_ond->naam, $vanaf);
	...
	public function update($p_loid, $p_kolom, $p_waarde, $p_reden="") {
		...
		if ($p_kolom == "Vanaf") {
			$sl = (new cls_lid($this->lidid, $p_waarde))->soortlid;
		} else {
			$sl = "";
		}
		...
		} elseif ($p_kolom == "Vanaf" and $this->i_ond->alleenleden == 1 and $sl !== "Lid") {
	...
	public function autogroepenbijwerken($p_altijdlog=0, $p_interval=180, $p_ondid=-1) {
...
//			$exec_tijd = (microtime(true) - $starttijd);
//			debug(__FUNCTION__ . "/Logboek: " . $exec_tijd, 2, 99);
		}

		$t = strtotime(sprintf("-%d minutes", $p_interval));
		if ($lb < date("Y-m-d H:i:s", $t) or $p_ondid == -2) {
		
			$f = "LENGTH(O.MySQL) > 10 AND LEFT(O.MySQL, 7)='SELECT '";
			if ($p_ondid == -2 and strlen($_SESSION['settings']['login_beperkttotgroep']) > 0) {
				$f .= sprintf(" AND O.RecordID IN (%s)", str_replace(";", ",", $_SESSION['settings']['login_beperkttotgroep']));
			} elseif ($p_ondid > 0) {
	...
	public function auto_einde($p_ondid=-1, $p_interval=180, $p_lidid=-1) {
		$starttijd = microtime(true);
...
//		debug(__CLASS__ . "->" . __FUNCTION__ . " / " . $lagb);
			...
			if ($p_lidid > 0) {
				$this->where = sprintf("LO.Lid=%d", $p_lidid);
			} else {
				$this->where = "";
			}
			$ondrows = $i_ond->lijst(1, $f);
			foreach ($ondrows as $ondrow) {
				$lorows = $this->lijst($ondrow->RecordID, 3, "LO.Lid, LO.Vanaf");
				foreach ($lorows as $lorow) {
					$this->vulvars($lorow->RecordID);
					$query = sprintf("SELECT MAX(IFNULL(LM.Opgezegd, '9999-12-31')) FROM %sLidmaatschap AS LM WHERE LM.Lid=%d;", TABLE_PREFIX, $lorow->LidID);
					$eld = $this->scalar($query);
					if ($eld < "9999-12-31" and (($ondrow->Type == "A" and $eld <= date("Y-m-d", strtotime("+6 month"))) or $eld < date("Y-m-d"))) {
						$this->update($lorow->RecordID, "Opgezegd", $eld, sprintf("het lidmaatschap per %s is beëindigd.", $eld));
						$rv++;
					}
				}
			}
	...
	public function vulvars($p_actid=-1) {
		...
		if ($this->actid > 0) {
			...
			if (isset($row->RecordID)) {
				...
				$this->gbr = $row->GBR ?? "";
				$this->beperkingaantal = $row->BeperkingAantal ?? 0;
				$this->ingevoerd = $row->Ingevoerd ?? "";
				$this->gewijzigd = $row->Gewijzigd ?? "";
			} else {
				$this->actid = 0;
...
class cls_Groep extends cls_db_base {
	...
	public string $code = "";
	public string $omschrijving = "";
	public string $instructeurs = "";
	public int $dpid = 0;
	public string $starttijd = "";
	public string $eindtijd = "";
	public int $aanwezigheidsnorm = 0;
	...
	public function vulvars($p_afdid=-1, $p_grid=-1) {
		...
		if ($this->grid > 0) {
			...
			if (isset($row->RecordID) and $row->RecordID > 0) {
				...
				$this->aanwezigheidsnorm = $row->Aanwezigheidsnorm ?? 0;
			} else {
				$this->grid = 0;
			}
		}
		...
		} elseif (strlen(trim($this->omschrijving)) > 0) {
			$this->naam = $this->omschrijving;
		} elseif ($this->dpid > 0) {
			$this->naam = $this->i_dp->naam;
		} else {
			$this->naam = sprintf("Groep %d", $this->grid);
		}
	...
	public function lijst($p_afdid=-1, $p_order="GR.Starttijd, GR.Volgnummer, GR.Omschrijving, GR.RecordID") {
		
		$query = sprintf("SELECT GR.*
								FROM %2\$s
								WHERE (GR.OnderdeelID=%3\$d OR GR.RecordID=0)
								ORDER BY %4\$s;", TABLE_PREFIX, $this->basefrom, $p_afdid, $p_order);
		$result = $this->execsql($query);
		return $result->fetchAll();		
	}
	
	public function selectlijst($p_afdid=-1) {
		...
		$query = sprintf("SELECT GR.*,
						  ...
						  (SELECT COUNT(*) FROM %1\$sLidond AS LO WHERE LO.GroepID=GR.RecordID AND IFNULL(LO.Opgezegd, '9999-12-31') >= '%4\$s' AND LO.OnderdeelID=%3\$d) AS aantalInGroep
						  FROM %2\$s
						  WHERE (GR.OnderdeelID=%3\$d OR GR.RecordID=0)
						  ORDER BY GR.Starttijd, GR.Volgnummer, GR.Omschrijving, GR.Kode, GR.RecordID;", TABLE_PREFIX, $this->basefrom, $p_afdid, $this->per);
		$result = $this->execsql($query);
		return $result->fetchAll();
	...
	public function update($p_grid, $p_kolom, $p_waarde, $p_reden="") {
		...
		if ($p_kolom == "Volgnummer" and (intval($p_waarde) <= 0 or strlen($p_waarde) == 0)) {
			$p_waarde = 0;
		} elseif ($p_kolom == "Aanwezigheidsnorm" and (intval($p_waarde) <= 0 or strlen($p_waarde) == 0)) {
			$p_waarde = "NULL";
		} elseif ($p_kolom == "Aanwezigheidsnorm" and intval($p_waarde) > 100) {
			$p_waarde = 100;
		} elseif (($p_kolom == "Starttijd" or $p_kolom == "Eindtijd") and strlen($p_waarde) == 4 and $p_waarde != "NULL" and substr($p_waarde, 0, 1) != "0") {
			$p_waarde = "0" . $p_waarde;
		}
	...
	public function controle() {
		...
		foreach($result->fetchAll() as $row) {
			...
			} elseif ($row->Starttijd != null and strlen($row->Starttijd) > 0 and validTime($row->Starttijd) == false) {
				$this->update($row->RecordID, "Starttijd", "NULL", "de starttijd geen geldige tijd is.");
			} elseif ($row->Eindtijd != null and strlen($row->Eindtijd) > 0 and validTime($row->Eindtijd) == false) {
				$this->update($row->RecordID, "Eindtijd", NULL, "de eindtijd geen geldige tijd is.");
			} elseif ($row->Eindtijd != null and strlen($row->Eindtijd) > 0 and $row->Eindtijd < $row->Starttijd) {
				$this->update($row->RecordID, "Eindtijd", NULL, "de eindtijd voor de starttijd ligt.");
			}
		}
	}  # cls_Groep->controle
...
class cls_Login extends cls_db_base {
	...
	public string $activatiekey = "";
	public int $foutelogin = 0;
	...
	public function vulvars($p_lidid=-1, $p_email="", $p_lidnr=0) {
		if ($p_lidid >= 0) {
			$this->lidid = $p_lidid;
		}
		...
		if ($this->lidid <= 0 && strlen($this->filteremail) > 5) {
			...
			$rows = $this->execsql($query)->fetchAll();		
			if (count($rows) == 0) {
				$this->mess = sprintf("E-mailadres '%s' is onbekend in onze administratie.", $p_email);
				$this->lidid = 0;
			} elseif (count($rows) > 1) {
				$this->mess = sprintf("E-mailadres '%s' is aan meerdere leden gekoppeld. Deze kan niet voor login worden gebruikt.", $p_email);
				$this->lidid = -1;
			} else {
		...
		if ($p_lidnr > 0 && $this->lidid <= 0) {
			$query = sprintf("SELECT IFNULL(MAX(LM.Lid), 0) FROM %sLidmaatschap AS LM WHERE LM.Lidnr=%d AND IFNULL(LM.Opgezegd, '9999-12-31') >= '%s';", TABLE_PREFIX, $p_lidnr, $this->per);
		...
		$this->wachtwoord = "";
		if ($this->lidid > 0) {
			$query = sprintf("SELECT Login.* FROM %s WHERE Login.LidID=%d;", $this->basefrom, $this->lidid);
			$row = $this->execsql($query)->fetch();
			if (isset($row->RecordID)) {
				$this->loginid = $row->RecordID;
				$this->login = $row->Login;
				$this->wachtwoord = $row->Wachtwoord ?? "";
				$this->activatiekey = $row->ActivatieKey ?? "";
				$this->foutelogin = $row->FouteLogin ?? 0;
				$this->ingevoerd = $row->Ingevoerd ?? "";
				$this->gewijzigd = $row->Gewijzigd ?? "";
			} else {
				$this->loginid = 0;
	...
	public function lididherstel($p_login, $p_lidnr, $p_email) {
		...
		if ($this->lidid <= 0 && strlen($p_login) >= 5) {
			...
			if ($this->lidid == 0) {
				...
				$this->vulvars($this->lidid);
			}
		}
		
		if ($this->lidid > 0 && $this->i_lid->inloggentoegestaan == true) {
			$rv = $this->lidid;
	...
	public function nuingelogd() {
		...
		return $rv;
	}  # cls_Login->nuingelogd
	
	public function aanvragenmag($p_lidid=-1) {
		$this->vulvars($p_lidid);
		return $this->i_lid->inloggentoegestaan;
	}  # cls_Login->aanvragenmag

	public function lididbijlogin($p_login) {
		$query = sprintf("SELECT LidID FROM %s WHERE Login='%s';", $this->basefrom, $p_login);
		$row = $this->execsql($query)->fetch();
		if (isset($row->LidID)) {
			$this->vulvars($row->LidID);
		} else {
			$this->loginid = 0;
			$this->lidid = 0;
		}
	...
	}  # cls_Login-lididbijlogin

	public function controle($p_wachtwoord, $p_fromcookie=0) {
		
		$login = $_SESSION['username'] ?? "";
		$autounlock = $_SESSION['settings']['login_autounlock'] ?? 0;
		
		if ($autounlock > 0) {
			$query = sprintf("SELECT Login, LidID FROM %s WHERE Gewijzigd < DATE_SUB(SYSDATE(), INTERVAL %d MINUTE) AND FouteLogin > 0;", $this->basefrom, $autounlock);
			$result = $this->execsql($query);
			foreach ($result->fetchAll() as $row) {
		...
		if (isValidMailAddress($login, 0)) {
			$query = sprintf("SELECT Login.Login, Login.LidID FROM %1\$sLid AS L INNER JOIN %2\$s ON Login.LidID=L.RecordID WHERE (LOWER(L.Email) LIKE '%3\$s' OR LOWER(L.EmailVereniging) LIKE '%3\$s')", TABLE_PREFIX, $this->basefrom, strtolower($login));
			$rows = $this->execsql($query)->fetchAll();
			if (count($rows) == 1) {
				$login = $rows[0]->Login;
			}
	...
	public function valideerlogin($p_lidid, $p_key) {
		...
		if ($this->loginid == 0) {
			$this->mess = sprintf("Dit lid (%d) heeft geen login.", $p_lidid);
		} else {
			$query = sprintf("UPDATE %s SET ActivatieKey='', Gewijzigd=SYSDATE() WHERE ActivatieKey='%s' AND LidID=%d;", $this->table, $p_key, $this->lidid);
			$result = $this->execsql($query);
			if ($result == 1) {
				$this->mess = "Login is gevalideerd en kan worden gebruikt.";
			} else {
				$this->mess = "Het valideren van de login is niet gelukt. Mogelijk is deze login al gevalideerd. Zo niet probeer het later opnieuw.";
			}
		}
		$this->tas = 4;
		$this->log(0, 1);
...
class cls_Mailing extends cls_db_base {
	...
	public string $tocc = "";
	public int $ccafdelingen = 0;
	public int $template = 0;
	public int $htmldirect = 0;
	public int $zonderbriefpapier = 0;
	public int $zichtbaarvoor = 0;
	public int $evenementid = 0;
	public string $deleted_on = "";
	
	public string $zichtbaarwhere = "";
	public int $aantalontvangers = 0;
	public string $evenementoms = "";
	public int $ingevoerddoor = 0;
	...
	function __construct($p_mid=-1) {
		...
		$this->per = date("Y-m-d");
		$this->vulvars($p_mid);
		if (WEBMASTER == false and strlen($_SESSION['lidgroepen']) > 0 and in_array($_SESSION['settings']['mailing_alle_zien'], explode(",", $_SESSION['lidgroepen'])) == false) {
	...
	private function vulvars($p_mid=-1, $p_beperkt=0) {
		...
		$this->subject = "";
		if ($this->mid > 0) {
			$query = sprintf("SELECT M.* FROM %s WHERE M.RecordID=%d;", $this->basefrom, $this->mid);
			$row = $this->execsql($query)->fetch();
			if (isset($row->RecordID)) {
				$this->mailingvanafid = $row->MailingVanafID ?? 0;
				...
				$this->ccafdelingen = $row->CCafdelingen ?? 0;
				$this->template = $row->template ?? 0;
				$this->htmldirect = $row->HTMLdirect ?? 0;
				$this->zonderbriefpapier = $row->ZonderBriefpapier ?? 0;
				$this->zichtbaarvoor = $row->ZichtbaarVoor ?? 0;
				$this->evenementid = $row->EvenementID ?? 0;
				$this->deleted_on = $row->deleted_on?? ""; 
				$this->ingevoerd = $row->Ingevoerd ?? "";
				$this->gewijzigd = $row->Gewijzigd ?? "";
			} else {
				$this->mid = 0;
	...
	public function lijst($p_filter="") {
		...
		$query = sprintf("SELECT M.RecordID, CONCAT(M.subject, ' & ', IFNULL(M.Opmerking, '')) AS Onderwerp_Opmerking, MV.Vanaf_naam, M.subject, M.Opmerking, M.OmschrijvingOntvangers,
					CONCAT(MV.Vanaf_naam, ' & ', M.OmschrijvingOntvangers) AS Van_Aan%2\$s,
	...
	public function htmloptions($p_cv=-1) {
		...
		foreach ($this->lijst($f) as $row) {
			...
			if (strlen($this->omschrijvingontvangers) > 0) {
				$o .= " | " . $this->omschrijvingontvangers;
			}
			if ($this->aantalontvangers > 1) {
				$o .= sprintf(" (%d ontvangers)", $this->aantalontvangers);
			}
			$rv .= sprintf("<option value=%d%s>%s</option>\n", $row->RecordID, checked($p_cv, "option", $row->RecordID), $o);
		}
		return $rv;
	...
	public function mogelijkeontvangers($p_mid, $p_filter=0) {
		...
		*/
		...
		$result = $this->execsql($query);
		return $result->fetchAll();
	}  # cls_Mailing->mogelijkeontvangers
...
class cls_Mailing_hist extends cls_db_base {
	...
	private function vulvars($p_mhid=-1, $p_mid=-1) {
		...
		if ($this->mhid > 0) {
			$query = sprintf("SELECT MH.* FROM %s WHERE MH.RecordID=%d;", $this->basefrom, $this->mhid);
			$result = $this->execsql($query);
			$row = $result->fetch();
			if (isset($row->RecordID)) {
				$this->lidid = $row->LidID ?? 0;
				$this->mid = $row->MailingID ?? 0;
				$this->vanafid = $row->VanafID ?? 0;
				$this->subject = $row->subject ?? "";
				$this->message = $row->message ?? "";
				$this->xtrachar = $row->Xtra_Char ?? "";
				$this->xtranum = $row->Xtra_Num ?? 0;
				$this->to_addr = $row->to_addr ?? "";
				$this->aannaam = $row->to_name ?? "";
				$this->tocc = $row->cc_addr ?? "";
				$this->replyid = $row->ReplyID ?? 0;
				$this->ingevoerd = $row->Ingevoerd ?? "";
				$this->ingevoerddoor = $row->IngevoerdDoor ?? 0;
				$this->nietversturenvoor = $row->NietVersturenVoor ?? "";
				$this->verstuurd = $row->send_on ?? "";
				$this->zichtbaarvoor = $row->ZichtbaarVoor ?? 0;
				$this->zonderbriefpapier = $row->ZonderBriefpapier ?? 0;
			} else {
				$this->mhid = 0;
		...
		$this->i_lid = new cls_Lid($this->lidid);
		$this->i_mv = new cls_Mailing_vanaf($this->vanafid);
		
		$this->naamlogging = $this->subject;
		if ($this->lidid > 0) {
			$this->aannaam = htmlentities($this->i_lid->naam);
		} else {
			$this->aannaam = $this->to_addr;
		}
		
	}  # cls_Mailing_hist->vulvars
	...
	public function lijst($p_mid=-1) {
		...
		if ($this->mid > 0) {
			$query = sprintf("SELECT MH.RecordID, IF(MH.send_on > '2000-01-01', MH.send_on, 'In outbox') AS send_on, %1\$s AS Aan, M.subject
						...
						WHERE MH.MailingID=%4\$d %5\$s %6\$s
						ORDER BY MH.send_on DESC, MH.RecordID DESC;", $this->selectmhaan, $this->basefrom, TABLE_PREFIX, $this->mid, $this->zichtbaarwhere, $w);
		} else {
			$query = sprintf("SELECT MH.RecordID, MH.send_on, MV.Vanaf_naam, %1\$s AS Aan, MH.subject
						...
						WHERE MH.send_on > '2000-01-01' %4\$s %5\$s
						ORDER BY MH.send_on DESC, MH.RecordID DESC LIMIT %6\$d;", $this->selectmhaan, $this->basefrom, TABLE_PREFIX, $this->zichtbaarwhere, $w, $this->limit);
		}
	...
	public function controle() {
		...
		$this->mess = sprintf("%s->%s uitgevoerd.", __CLASS__, __FUNCTION__);
		$this->log(0, 1);
		
	}  # cls_Mailing_hist->controle
	...
	public function opschonen() {
		...
		if ($mho > 3) {
			$aant = 0;
			$query = sprintf("SELECT MH.RecordID FROM %s WHERE MH.send_on < DATE_SUB(CURDATE(), INTERVAL %d MONTH) AND (MH.send_on IS NOT NULL) LIMIT 750;", $this->basefrom, $mho);
			$result = $this->execsql($query);
		...
		if ($mho > 1) {
			$query = sprintf("SELECT LM.RecordID, LM.Lid FROM %1\$sLidmaatschap AS LM WHERE IFNULL(LM.Opgezegd, '9999-12-31') < DATE_SUB(CURDATE(), INTERVAL %2\$d MONTH) AND LM.Lid IN (SELECT MH.LidID FROM %1\$sMailing_hist AS MH) LIMIT 250;", TABLE_PREFIX, $mho);
			$result = $this->execsql($query);
			foreach ($result->fetchAll() as $lmrow) {
				$i_lm->vulvars($lmrow->RecordID);
				if ($i_lm->i_lid->eindelaatstelidmaatschap < date("Y-m-d", strtotime(sprintf("-%d month", $mho)))) {
					$query = sprintf("DELETE FROM %s WHERE LidID=%d AND send_on < DATE_SUB(CURDATE(), INTERVAL %d MONTH);", $this->table, $lmrow->Lid, $mho);
					$aant = $this->execsql($query);
					if ($aant > 0) {
						$this->lidid = $lmrow->Lid;
						$this->mess = sprintf("%s: %d verzonden e-mails van voormalig lid %d verwijderd.", $this->table, $aant, $lmrow->Lid);
						$this->log(0, 1);
					}
				}
			}
...
class cls_Mailing_rcpt extends cls_db_base {
	...
	public function vulvars($p_mrid=-1, $p_mid=-1, $p_lidid=-1) {
		if ($p_mrid >= 0) {
			$this->mrid = $p_mrid;
		}
		if ($p_mid >= 0) {
		...
		$this->naamlogging = $this->i_ml->subject;
	...
	public function lijst($p_mid=-1, $p_lidid=-1) {
		...
		if ($p_lidid > 0) {
			$w .= sprintf(" AND MR.LidID=%d", $p_lidid);
		}
		
		$query = sprintf("SELECT MR.LidID, MR.RecordID, MR.to_address, MR.MailingID, MR.Xtra_Char, MR.Xtra_Num,
						  %s AS NaamLid, %s AS Zoeknaam_lid, L.Adres, L.Postcode, L.Woonplaats, L.Email, L.EmailVereniging, L.EmailOuders, L.GEBDATUM
	...
	public function aantalontvangers($p_mid=-1, $p_lidid=-1) {
		...
		debug(__CLASS__ . "->" . __FUNCTION__ . " heroverwegen", 0, 99);
...
class cls_Mailing_vanaf extends cls_db_base {
	...
	public function vulvars($p_mvid=-1, $p_email="") {
		...
		if ($this->mvid > 0) {
			...
			$row = $result->fetch();
			if (isset($row->RecordID)) {
				$this->vanaf_email = $row->Vanaf_email ?? "";
	...
	public function lijst($p_fetched=1) {
		$query = sprintf("SELECT MV.*, (SELECT COUNT(*) FROM %1\$sMailing AS M WHERE MV.RecordID=M.MailingVanafID) AS aantalMailings, IF((SELECT COUNT(*) FROM %1\$sMailing_hist AS MH WHERE MH.VanafID=MV.RecordID)=0, MV.RecordID, 0) AS delrid FROM %2\$s ORDER BY MV.Vanaf_email;", TABLE_PREFIX, $this->basefrom);
		$result = $this->execsql($query);
		if ($p_fetched == 1) {
...
// Controle eigen lijsten
		$query = sprintf("DELETE FROM %s WHERE DatumTijd < DATE_SUB(CURDATE(), INTERVAL 3 MONTH) AND TypeActiviteit=23 AND TypeActiviteitSpecifiek > 1;", $this->table);
		$this->execsql($query, 2);
...
class cls_Diploma extends cls_db_base {
	...
	public int $dpvolgende = 0;
	public string $eindeuitgifte = "";
	public int $geldigheid = 0;					// Hoelang is dit diploma na behalen geldig. In maanden.
	public int $historieopschonen = 0;
	public string $vervallen = "";
	public int $zelfservice = 0;				// Is dit diploma muteerbaar in de zelfservice?
	public int $afdelingsspecifiek = 0;			// Optie: de afdeling waar dit diploma bij hoort.
	
	public string $naamlang = "";				// Naam met uitgevende instantie	
	public int $aantalhouders = 0;				// Het aantal leden dat dit diploma nu nog heeft en waarvan het geldig is.
	public int $aantalonderdelen = 0;			// Het aantal examenonderdelen
	public string $naamvoorganger = "";			// Naam van het vorige diploma
	public string $naamvolgende = "";			// Naam van het opvolgende diploma
	...
	public function vulvars($p_dpid=-1) {
		...
		if ($this->voorgangerid > 0) {
			$f = sprintf("DP.RecordID=%d", $this->voorgangerid);
			$this->naamvoorganger = $this->max("Naam", $f);
		} else {
			$this->naamvoorganger = "";
		}
		...
		if ($this->organisatie > 0) {
			$this->naamlang = $this->i_org->naam . " " . $this->naam;
		} else {
			$this->naamlang = $this->naam;
		}
...
class cls_Liddipl extends cls_db_base {
	...
	private int $ldid = 0;
	private int $dpid = 0;
	public string $datbehaald = "";
	private int $examen = 0;
	public int $examengroep = 0;
	private int $geslaagd = 0;
	private int $laatstebeoordeling = 1;
	public string $licentievervallenper = "";
	
	public string $vervaltper = "";				// Het resultaat van licentievervallenper, datbehaald en i_dp->geldigheid
	...
	public function vulvars($p_ldid=-1, $p_lidid=-1, $p_dpid=-1) {
		...
		if ($this->ldid > 0) {
			...
			if (isset($row->RecordID) and $row->RecordID > 0) {
				...
				$this->examengroep = $row->Examengroep ?? 0;
				$this->geslaagd = $row->Geslaagd ?? 0;
				$this->laatstebeoordeling = $row->LaatsteBeoordeling ?? 1;
				$this->licentievervallenper = $row->LicentieVervallenPer ?? "";
			} else {
				$this->ldid = 0;
			}	
		}
		...
		$this->i_dp = new cls_Diploma($this->dpid);
		$this->i_lid = new cls_Lid($this->lidid);
		$this->i_ex = new cls_Examen($this->examen);
		...
		if (strlen($this->licentievervallenper) >= 10 and $this->licentievervallenper <= $this->datbehaald) {
			$this->ldclass = "deleted";
		} elseif (strlen($this->licentievervallenper) >= 10 and $this->licentievervallenper < date("Y-m-d")) {
			$this->ldclass = "vervallen";
		} else {
			$this->ldclass = "";
		}
		$this->ldtitle = "";
		...
		if ($this->i_dp->voorgangerid > 0 and $this->lidid > 0 and (strlen($this->i_dp->eindeuitgifte) < 10 or $this->i_dp->eindeuitgifte > date("Y-m-d"))) {
			
			$this->where = sprintf("LD.Lid=%d AND LD.DiplomaID=%d AND LD.Geslaagd=1 AND LD.LaatsteBeoordeling=1", $this->lidid, $this->i_dp->voorgangerid);
			$dvd = $this->max("DatumBehaald");
			if (strlen($dvd) < 10) {
		...
		if ($this->i_dp->voorgangerid > 0 and $this->geslaagd == 0 and (strlen($this->i_dp->eindeuitgifte) < 10 or $this->i_dp->eindeuitgifte > date("Y-m-d"))) {
			$i_ak = new cls_Afdelingskalender();
			if ($this->i_dp->afdelingsspecifiek > 0) {
				$i_ak->where = sprintf("AK.OnderdeelID=%d", $this->i_dp->afdelingsspecifiek);
			}
			$f = sprintf("LD.Lid=%d AND LD.DiplomaID=%d AND LD.Geslaagd=1 AND LD.LaatsteBeoordeling=1", $this->lidid, $this->i_dp->voorgangerid);
			$dvd = $this->max("DatumBehaald", $f);
			if (strlen($dvd) == 10) {
				$f = sprintf("AK.Datum > '%s' AND AK.Datum < CURDATE()", $dvd);
				$wdlt = $i_ak->aantal($f);
				
				if ($wdlt == 0) {
				    $from = new DateTime($dvd);
					$today = new DateTime();
					$wdlt = floor($from->diff($today)->days/7);
				}
				
				if ($this->i_dp->doorlooptijd > 0 and $wdlt > $this->i_dp->doorlooptijd) {
					$this->ldclass .= " voortgangsprobleem";
					if (strlen($this->ldtitle) > 0) {
						$this->ldtitle .= ", ";
					}
				}
				$this->ldtitle .= sprintf("%s behaald op %s", (new cls_diploma())->naam($this->i_dp->voorgangerid), date("d-m-Y", strtotime($dvd)));
				if ($wdlt > 1) {
					$this->ldtitle .= sprintf(" (%d lessen)", $wdlt);
				}
			}
			$i_ak = null;
		}
		
		if (strlen($this->licentievervallenper) == 10) {
			$this->vervaltper = $this->licentievervallenper;
		} elseif ($this->i_dp->geldigheid == 0) {
			$this->vervaltper = "9999-12-31";
		} else {
			$hd = new datetime($this->datbehaald);
			$hd->modify(sprintf("+%d month", $this->i_dp->geldigheid));
	...
	public function overzichtperdiploma($p_dpid) {
		...
		$query = sprintf("SELECT LD.*, %1\$s AS NaamLid, L.GEBDATUM, %2\$s AS GeldigTot
						  FROM (%3\$s INNER JOIN %4\$sDiploma AS DP ON LD.DiplomaID=DP.RecordID) INNER JOIN %4\$sLid AS L ON LD.Lid=L.RecordID
						  WHERE LD.DatumBehaald > '1900-01-01' AND LD.DiplomaID=%5\$d AND LD.Geslaagd=1 AND LD.LaatsteBeoordeling=1 
						  ORDER BY L.Achternaam, L.TUSSENV, L.Roepnaam, LD.DatumBehaald;", $this->selectnaam, $this->sqlgeldigtot, $this->basefrom, TABLE_PREFIX, $p_dpid);
		$result = $this->execsql($query);
	...
	public function overzichtperexamen($p_examen, $p_diploma=-1) {

		$w = sprintf("LD.Examen=%d", $p_examen);
		if ($p_diploma > 0) {
			$w .= sprintf(" AND LD.DiplomaID=%d", $p_diploma);
		}
		if (strlen($this->where) > 0) {
	...
	public function perexamendiploma($p_examen, $p_dpid, $p_alleengeslaagd=0) {
		...
		$query = sprintf("SELECT LD.*, %s AS NaamLid, %s AS Zoeknaam, L.GEBDATUM, L.GEBPLAATS, L.RelnrRedNed
						  FROM %s INNER JOIN %sLid AS L ON LD.Lid=L.RecordID
						  WHERE LD.Examen=%d AND LD.DiplomaID=%d%s
						  ORDER BY LD.Examengroep, L.Achternaam, L.TUSSENV, L.Roepnaam, LD.DatumBehaald;", $this->selectnaam, $this->selectzoeknaam, $this->basefrom, TABLE_PREFIX, $p_examen, $p_dpid, $xw);
		$result = $this->execsql($query);
	...
	public function add($p_lidid, $p_dpid, $p_exdatum="", $p_examen=0) {
		...
		if ($this->lidid <= 0) {
			...
			if ($p_examen > 0 and $this->scalar($dubqry_ex) > 0)  {
				...
				$query = sprintf("INSERT INTO %s (RecordID, Lid, DiplomaID, Examen, Examengroep, DatumBehaald, Geslaagd, LaatsteBeoordeling, Ingevoerd) VALUES (%d, %d, %d, %d, %s, %s, %d, %d, SYSDATE());", $this->table, $nrid, $this->lidid, $this->dpid, $p_examen, $exgr, $db, $gs, $lb);
				if ($this->execsql($query) > 0) {
					$this->mess = sprintf("Tabel Liddipl: Record %d (%s) is toegevoegd.", $nrid, $this->i_dp->naam);
				} else {
					$nrid = 0;
	...
	public function update($p_ldid, $p_kolom, $p_waarde, $p_reden="") {
		...
		} elseif ($p_kolom == "DatumBehaald" and $p_waarde < $this->i_lid->geboortedatum and $p_waarde > "1900-01-01") {
	...
	public function controle($p_exid=-1, $p_dpid=-1) {
		
		$w = "";
		if ($p_exid > 0) {
			$this->where = sprintf("LD.Examen=%d", $p_exid);
		} elseif ($p_dpid > 0) {
			$this->where = sprintf("LD.DiplomaID=%d", $p_dpid);
		} else {
			$this->where = "";
		}
		...
		foreach ($this->basislijst() as $row) {
			$this->vulvars($row->RecordID);
			if (strlen($this->i_lid->overleden) == 10 and ($this->licentievervallenper < "1970-01-01" or $this->i_lid->overleden < $this->licentievervallenper)) {
				$this->update($row->RecordID, "LicentieVervallenPer", $this->i_lid->overleden, "het lid overleden is");
			}  elseif (strlen($this->i_dp->vervallen) >= 10 and $this->i_dp->vervallen > $this->licentievervallenper) {
				$this->update($row->RecordID, "LicentieVervallenPer", $this->i_dp->vervallen, "het diploma vervallen is.");
			}  elseif (strlen($this->licentievervallenper) < 10 and $this->i_dp->geldigheid > 0 and $this->vervaltper < date("Y-m-d")) {
				$this->update($row->RecordID, "LicentieVervallenPer", $this->vervaltper, "de geldigheid van dit diploma beperkt is");
			} elseif ($this->examen > 0 and $this->i_ex->exid == 0) {
				$this->update($row->RecordID, "Examen", "NULL", "het examen niet bestaat");
			} elseif ($this->examen > 0 and $this->i_ex->proef == 0 and $this->laatstebeoordeling == 0) {
				$this->update($row->RecordID, "LaatsteBeoordeling", 1, "bij een examen is het altijd een laatste beoordeling.");
			} elseif ($this->examen > 0 and $this->i_ex->datum != $this->datbehaald and strlen($this->i_ex->datum) == 10 and $this->geslaagd == 1 and $this->laatstebeoordeling == 1) {
				$this->update($row->RecordID, "DatumBehaald", $this->i_ex->datum);
			} elseif ($this->examen == 0 and $this->geslaagd == 0) {
	...
	public function opschonen() {
		...
		$a = 0;
		if ($bt > 0) {
			$i_lid->per = sprintf("-%d month", $bt);
			foreach ($i_lid->ledenlijst(3) as $lrow) {
				$i_lid->vulvars($lrow->RecordID);
				$this->lidid = $i_lid->lidid;
				if ($i_lid->eindelaatstelidmaatschap < date("Y-m-d", strtotime(sprintf("-%d month", $bt)))) {
					$query = sprintf("SELECT LD.* FROM %s WHERE LD.Lid=%d;", $this->basefrom, $this->lidid);
					foreach ($this->execsql($query)->fetchAll() as $ldrow) {
						$reden = sprintf("het lidmaatschap op %s is beëindigd.", $i_lid->eindelaatstelidmaatschap);
						if ($a < 125) {
...
class cls_Examen extends cls_db_base {
	...
	public int $onderdeelid = 0;
	public int $proef = 0;
	
	public string $examenoms = "Examen";
	public string $examenomskort = "Examen";
	public int $aantalkandidaten = 0;
	
	private object $i_ond;
	...
	public function vulvars($p_exid, $p_datum="", $p_onderdeel=0) {
		...
		if ($this->exid > 0) {
			...
			$row = $this->execsql($query)->fetch();
			if (isset($row->Nummer)) {
				$this->onderdeelid = $row->OnderdeelID ?? 0;
				$this->datum = $row->Datum ?? "";
				$this->plaats = $row->Plaats ?? "";
				$this->begintijd = $row->Begintijd ?? "";
				$this->proef = $row->Proefexamen ?? 0;
				if ($this->proef == 1) {
		...
		$this->i_ond = new cls_Onderdeel($this->onderdeelid);
		
		if ($this->i_ond->organisatie == 2) {
			if ($this->proef == 1) {
		...
		$this->examenoms = $this->examenomskort . " " . $this->i_ond->code;
	}  # cls_Examen->vulvars
	...
	public function opschonen() {
		...
		$result = $this->execsql($query);
		$reden = "het examen in het verleden ligt en er zijn geen gekoppelde records";
	...
	public function vulvars($p_evid=-1) {
		...
		if ($this->evid > 0) {
			...
			if (isset($evrow->RecordID)) {
				...
				$this->maxdeelnemers = $evrow->MaxDeelnemers ?? 0;
	...
	public function vulvars($p_evid=-1, $p_lidid=-1, $p_edid=-1) {
		...
		if ($this->edid > 0) {
			...
			if (isset($row->RecordID)) {
				...
				$this->lidid = $row->LidID ?? 0;
				$this->opmerking = trim(str_replace("\"", "'", $row->Opmerking ?? ""));
				$this->functie = trim(str_replace("\"", "'", $row->Functie ?? ""));
		...
		$this->statusoms = "";
		$this->aanwezig = false;
		if (strlen($this->status) > 0 and $this->status != "G") {
		...
		if (WEBMASTER or in_array($this->i_ev->organisatie, explode(",", $_SESSION['lidgroepen']))) {
			$this->magmuteren = true;
		}
...
class cls_Artikel extends cls_db_base{
	...
	public int $maxperlid = 1;
	
	public string $codeomsmaat = "";
	public string $omsmaat = "";
	public int $voorraad = 0;
	public int $vrijevoorraad = 0;	
	...
	public function vulvars($p_artid=-1) {
		if ($p_artid >= 0) {
			$this->artid = $p_artid;
		}
		if ($this->artid > 0) {
			...
			if (isset($artrow->RecordID)) {
				...
				$this->maxperlid = $row->MaxAantalPerLid ?? 1;
			} else {
				$this->artid = 0;
			}
		}
		...
		$this->naamlogging = $this->omschrijving;
		if (strlen($this->maat) > 0) {
			$this->naamlogging .= " " . $this->maat;
		}
		
		$this->codeomsmaat = $this->code . " | " . $this->omschrijving;
		if (strlen($this->maat) > 0) {
			$this->codeomsmaat .= " (" . $this->maat . ")";
		}
				
		$this->omsmaat = $this->omschrijving;
		if (strlen($this->maat) > 0) {
			$this->omsmaat .= " (" . $this->maat . ")";
		}
		
		$query = sprintf("SELECT IFNULL(SUM(VB.Aantal), 0) FROM %sWS_Voorraadboeking AS VB WHERE VB.ArtikelID=%d;", TABLE_PREFIX, $this->artid);
		$this->voorraad = $this->scalar($query);
		
		$query = sprintf("SELECT IFNULL(SUM(VB.Aantal), 0) FROM %sWS_Voorraadboeking AS VB WHERE VB.ArtikelID=%d AND IFNULL(VB.OrderregelID, 0)=0;", TABLE_PREFIX, $this->artid);
		$this->vrijevoorraad = $this->scalar($query);
		$query = sprintf("SELECT IFNULL(SUM(ORD.AantalBesteld), 0) FROM %sWS_Orderregel AS ORD WHERE ORD.Artikel=%d;", TABLE_PREFIX, $this->artid);
		$this->vrijevoorraad = $this->vrijevoorraad - $this->scalar($query);
	}
	...
	public function lijst($p_type="bestellijst") {
		...
		$query .= " ORDER BY Art.Code, Art.Omschrijving, Art.Maat, Art.RecordID;";
		$result = $this->execsql($query);
		return $result->fetchAll();
	}
	...
	public function htmloptions($p_cv=-1, $p_filter="") {
		...
		$query = sprintf("SELECT Art.RecordID FROM %s", $this->basefrom);
		if ($p_filter == "bestelbaar") {
			$query .= sprintf(" WHERE IFNULL(Art.BeschikbaarTot, '9999-12-31') >= '%1\$s' AND IFNULL(Art.VervallenPer, '9999-12-31') >= '%1\$s' AND (Art.BeperkTotGroep IN (%2\$s))", $this->per, $_SESSION["lidgroepen"]);
			$query .= sprintf(" AND (IFNULL(Art.MaxAantalPerLid, 0)=0 OR Art.MaxAantalPerLid > (SELECT IFNULL(SUM(Ord.AantalBesteld), 0) FROM %sWS_Orderregel AS Ord WHERE Ord.Lid=%d AND Art.RecordID=Ord.Artikel))", TABLE_PREFIX, $_SESSION['lidid']);
			$query .= sprintf(" AND (SELECT IFNULL(COUNT(Ord.AantalBesteld), 0) FROM %sWS_Orderregel AS Ord WHERE Ord.Lid=%d AND Art.RecordID=Ord.Artikel) = 0", TABLE_PREFIX, $_SESSION['lidid']);
		...
		foreach ($result->fetchAll() as $row) {
			$this->vulvars($row->RecordID);
			$rv .= sprintf("<option value=%d %s>%s</option>\n", $row->RecordID, checked($p_cv, "option", $row->RecordID), $this->codeomsmaat);
		}
		return $rv;
	}
	...
	public function add($p_code="") {
		...
		$query = sprintf("INSERT INTO %s (RecordID, Code, MaxAantalPerLid, BeperkTotGroep) VALUES  (%d, '%s', 1, -2);", $this->table, $nrid, $p_code);
		if ($this->execsql($query) > 0) {
...
class cls_Orderregel extends cls_db_base {
	...
	public float $prijsperstuk = 0;
	public int $ordernr = 0;
	public string $bestellingdefinitief = "";
	public string $opmerking = "";
	
	public float $bedrag = 0;
	public int $aantalgeleverd = 0;
	
	private string $selectcodeomsmaat = "CONCAT(Art.Code, ' - ', Art.Omschrijving, IF(LENGTH(IFNULL(Art.Maat, '')) > 0, CONCAT(' (', Art.Maat, ')'), '')) AS CodeOmsMaat";
	
	public object $i_art;
	public object $i_lid;

	function __construct($p_lidid=-1, $p_orid=-1) {
	...
	public function vulvars($p_orid=0) {
		$this->orid = $p_orid;
		...
		if ($this->orid > 0) {
			...
			if (isset($row->RecordID)) {
				...
				$this->aantalbesteld = $row->AantalBesteld ?? 0;
				$this->prijsperstuk = $row->PrijsPerStuk ?? 0;
				$this->ordernr = $row->Ordernr ?? 0;
				$this->bestellingdefinitief = $row->BestellingDefinitief ?? "";
				$this->opmerking = $row->Opmerking ?? "";
				$this->ingevoerd = $row->Ingevoerd ?? "";
				$this->gewijzigd = $row->Gewijzigd ?? "";
			} else {
		...
		$this->i_art = new cls_Artikel($this->artid);
		$this->i_lid = new cls_Lid($this->lidid);
		
		if ($this->orid > 0) {
			$query = sprintf("SELECT IFNULL(SUM(VB.Aantal), 0) FROM %sWS_Voorraadboeking AS VB WHERE VB.OrderregelID=%d;", TABLE_PREFIX, $this->orid);
			$this->aantalgeleverd = ($this->scalar($query) * -1);
		} else {
			$this->aantalgeleverd = 0;
		}
		
		$this->naamlogging = $this->i_art->naamlogging;
		$this->bedrag = $this->prijsperstuk * $this->aantalbesteld;
	}
	...
	public function lijst($p_filter="", $p_eersterecord=0, $p_orderby="") {
		...
		$query = sprintf("SELECT Ord.RecordID, Ord.Ordernr, Ord.Lid, Ord.Artikel, Ord.AantalBesteld, Ord.PrijsPerStuk, Ord.Opmerking,
					...
					Art.Code, Art.Omschrijving, Art.Maat, CONCAT(Art.Omschrijving, ' ', IFNULL(Art.Maat, '')) as ArtOms, %6\$s,
					...
					%4\$s
					ORDER BY %5\$s IF(Ord.Artikel > 0 AND Ord.AantalBesteld > 0, 1, 0), L.Achternaam, L.Roepnaam, Ord.Ordernr, Art.Code;", $this->selectnaam, $this->basefrom, TABLE_PREFIX, $p_filter, $p_orderby, $this->selectcodeomsmaat);
		$result = $this->execsql($query);
		if ($p_eersterecord == 1) {
			return $result->fetch();
		} else {
	...
	public function bestelling($p_lidid, $p_artikelid, $p_filter="") {
		...
		$query = sprintf("SELECT * FROM %s WHERE Ord.Lid=%d AND Ord.Artikel=%d %s;", $this->basefrom, $p_lidid, $p_artikelid, $p_filter);
		$result = $this->execsql($query);
		return $result->fetch();
		
	}  # cls_Orderregel->bestelling
	...
	public function add($p_lidid, $p_artikelid, $p_incl_ordernr=0) {
		...
		$query = sprintf("INSERT INTO %s (RecordID, Lid, Artikel, AantalBesteld, Ordernr) VALUES (%d, %d, %d, 1, %d);", $this->table, $nrid, $p_lidid, $this->artid, $ordnr);
		if ($this->execsql($query) > 0) {
			$this->mess = sprintf("Orderregel %d met artikel '%s' is toegevoegd.", $nrid, $this->i_art->code);
			$this->log($nrid, 1);
...
class cls_Voorraadboeking extends cls_db_base {
	...
	public string $omschrijving = "";
	public int $aantal = 0;
	public int $orderregelid = 0;
	
	public object $i_art;
	public object $i_or;
	
	function __construct($p_vbid=-1) {
		parent::__construct();
	...
	public function vulvars($p_vbid=-1) {
		if ($p_vbid >= 0) {
			$this->vbid = $p_vbid;
		}
		...
		$this->orderregelid = 0;
		if ($this->vbid > 0) {
			$query = sprintf("SELECT VB.* FROM %s WHERE VB.RecordID=%d;", $this->basefrom, $this->vbid);
			$vbrow = $this->execsql($query)->fetch();
			if (isset($vbrow->RecordID)) {
				$this->omschrijving = $vbrow->Omschrijving ?? "";
				$this->artid = $vbrow->ArtikelID ?? 0;
				$this->datum = $vbrow->Datum ?? "";
				$this->aantal = $vbrow->Aantal ?? 0;
				$this->orderregelid = $vbrow->OrderregelID ?? 0;
			} else {
				$this->vbid = 0;
			}
		}
		...
		$this->i_art = new cls_Artikel($this->artid);
		$this->i_or = new cls_Orderregel(-1, $this->orderregelid);
	}
	
	public function lijst($p_art=-1) {
	...
	public function add($p_art, $p_orid=0) {
		...
		$query = sprintf("INSERT INTO %s (RecordID, ArtikelID, OrderregelID, Datum) VALUES (%d, %d, %d, CURDATE());", $this->table, $nrid, $p_art, $p_orid);
		if ($this->execsql($query) > 0) {
...
class cls_Rekening extends cls_db_base {	
	public $rkid = 0;
	private $reksel = "";
	public string $debnaam = "";
	public string $datum = "";
	public string $omschrijving = "";
	public int $seizoen = 0;
	public int $betaalddoor = 0;
	public string $opmerkingintern = "";
	public int $aantalbetaaltermijnen = 1;
	public int $dagenperbetaaltermijn = 30;
	
	public float $bedrag = 0;
	public float $betaald = 0;
	public string $uiterstebetaaldatum = "";
	public string $einde_eerstetermijn = "";
	public int $aantalledenoprekening = 1;
	...
	function __construct($p_rkid=-1) {
		...
		$this->alias = "RK";
		$this->basefrom = $this->table . " AS " . $this->alias;
		$this->pkkol = "Nummer";
		$this->per = date("Y-m-d");
		$this->vulvars($p_rkid);
		$this->ta = 14;
	...
	public function vulvars($p_rkid=-1) {
		...
		if ($this->rkid > 0) {
			...
			if (isset($rkrow->Nummer) and $rkrow->Nummer > 0) {
				...
				$this->opmerkingintern = $rkrow->OpmerkingIntern ?? "";
				$this->aantalbetaaltermijnen = $rkrow->BET_TERM ?? 1;
				$this->dagenperbetaaltermijn = $rkrow->BETAALDAG ?? 30;
				...
				$query = sprintf("SELECT COUNT(DISTINCT(RR.Lid)) FROM %sRekreg AS RR WHERE RR.Rekening=%d AND RR.Lid > 0;", TABLE_PREFIX, $this->rkid);
				$this->aantalledenoprekening = $this->scalar($query);
		...
		$ubd = new datetime($this->datum);
		$ubd->modify(sprintf("+%d day", $this->aantalbetaaltermijnen * $this->dagenperbetaaltermijn));
		$this->uiterstebetaaldatum = $dtfmt->format($ubd);

		$eet = new datetime($this->datum);
		$eet->modify(sprintf("+%d day", $this->dagenperbetaaltermijn));
		$this->einde_eerstetermijn = $dtfmt->format($eet);		
		
		$this->i_lid = new cls_Lid($this->lidid);
		$this->i_sz = new cls_Seizoen($this->seizoen);
		$this->i_bd = new cls_Lid($this->betaalddoor);
	...
	public function add($p_rkid, $p_seiz, $p_lidid, $p_datum="") {
		global $dbc;
		...
		if ($this->lidid <= 0) {
			...
			$data['omschrijving'] = $i_seiz->rekeningomschrijving ?? "Rekening";
			$data['lid'] = $this->lidid;
			$data['debnaam'] = $i_lid->naam;
			$data['betaalddoor'] = $this->lidid;
			$data['betaaldag'] = $i_seiz->betaaldagentermijn ?? 30;

			$query = sprintf("INSERT INTO %s (Nummer, Seizoen, Datum, OMSCHRIJV, Lid, DEBNAAM, BetaaldDoor, BETAALDAG, BET_TERM, Bedrag, Betaald) VALUES (:nummer, :seizoen, :datum, :omschrijving, :lid, :debnaam, :betaalddoor, :betaaldag, 1, 0, 0);", $this->table);
			$stmt = $dbc->prepare($query);
			if ($stmt->execute($data) > 0) {
				$this->mess = sprintf("Rekening %d voor lid %d (%s) is toegevoegd", $data['nummer'], $data['lid'], $data['debnaam']);
				$this->log($nwreknr);

				if ($i_lid->rekeningbetaalddoor > 0 and $i_lid->rekeningbetaalddoor != $data['betaalddoor']) {
...
class cls_Rekeningregel extends cls_db_base {
	...
	const FROMCOMPLEET = "(" . TABLE_PREFIX . "Rekreg AS RR LEFT OUTER JOIN " . TABLE_PREFIX . "Lid AS L ON RR.Lid=L.RecordID) INNER JOIN " . TABLE_PREFIX . "Rekening AS RK ON RR.Rekening=RK.Nummer";
	...
	public function vulvars($p_rrid=-1, $p_rkid=-1) {
		...
		if ($this->rrid > 0) {
			...
			if (isset($row->RecordID)) {
				...
				$this->lidid = $row->Lid ?? 0;
				$this->regelnr = $row->Regelnr ?? 0;
				$this->omschrijving = $row->OMSCHRIJV ?? "";
				$this->bedrag = $row->Bedrag ?? 0;
				$this->kostenplaats = $row->KSTNPLTS ?? "";
	...
	public function perrekening($p_rkid=-1, $p_xf="", $p_order="") {
		if ($p_rkid > 0) {
			$this->rkid = $p_rkid;
		}
		...
		$query = sprintf("SELECT RR.*, %s AS NaamLid, L.GEBDATUM, L.EmailOuders FROM %s WHERE RR.Rekening=%d%s ORDER BY %sRegelnr;", $this->selectnaam, self::FROMCOMPLEET, TABLE_PREFIX, $this->rkid, $p_xf, $p_order);
		$result = $this->execsql($query);
		return $result->fetchAll();
		
	}  # cls_Rekeningregel->perrekening
	...
	public function overzichtlid($p_lidid) {
		...
		$query = sprintf("SELECT DISTINCT IF((SELECT COUNT(*) FROM %1\$sRekreg AS RR WHERE RR.Rekening=RK.Nummer) > 0, RK.Nummer, 0) AS lnkNummer,
					...
					FROM %2\$s
					WHERE RR.Lid=%3\$d
					ORDER BY RK.Nummer DESC;", TABLE_PREFIX, self::FROMCOMPLEET, $this->lidid);
		$result = $this->execsql($query);
		return $result->fetchAll();
		
	}  # cls_Rekeningregel->overzichtlid
	...
	public function toevoegenstandaardregels($p_rkid, $p_lidid, $p_uitvoeren=1) {
		...
		if ($i_lid->islid or $i_lm->lidvanaf >= $this->i_rk->datum) {
			...
			$query = sprintf("SELECT COUNT(*) FROM %s WHERE RK.Seizoen=%d AND RR.KSTNPLTS='%s' AND RR.Lid=%d;", self::FROMCOMPLEET, $this->i_rk->seizoen, $this->i_rk->i_sz->verenigingscontributiekostenplaats, $this->lidid);
			if ($this->scalar($query) == 0) {
			...
			$i_lo->per = $this->i_rk->datum;
			foreach ($i_lo->lijstperlid($this->lidid, "A", 1) as $lorow) {
				$i_lo->vulvars($lorow->RecordID);
				$cb = 0;
				$query = sprintf("SELECT COUNT(*) FROM %s WHERE RK.Seizoen=%d AND RR.LidondID=%d;", self::FROMCOMPLEET, $this->i_rk->seizoen, $lorow->RecordID);
				if ($this->scalar($query) == 0) {
					if ($this->i_rk->i_sz->afdelingscontributieomschrijving == 2 and strlen($i_lo->i_gr->i_act->omschrijving) > 0) {
						$oms = $i_lo->i_gr->i_act->omschrijving;
					} else {
						$oms = $i_lo->i_ond->naam;
					}
					$kpl = $i_lo->i_ond->code;

					if ($i_lo->i_gr->actid > 0 and $i_lo->i_gr->i_act->contributie > 0) {
						if ($this->i_rk->i_sz->afdelingscontributieomschrijving == 3) {
							$oms .= " (" . $i_lo->i_gr->i_act->omschrijving . ")";
						}
						if (strlen($i_lo->i_gr->i_act->gbr) > 0) {
							$kpl = $i_lo->i_gr->i_act->gbr;
						} else {
							$kpl .= "-" . $i_lo->i_gr->i_act->code;
						}
						$cb = $i_lo->i_gr->i_act->contributie;
					}
					
					if ($i_lo->kader) {
						$cb = $i_lo->i_ond->contributiekaderlid;
					} elseif ($jl) {
						$cb += $i_lo->i_ond->contributiejeugdlid;
					} else {
						$cb += $i_lo->i_ond->contributielid;;
					}
					if ($i_lo->parttimepercentage > 0 and $i_lo->parttimepercentage != 100 and $cb != 0) {
						$oms .= sprintf(" (%d%%)", $i_lo->parttimepercentage);
						$cb = round(($cb/100) * $i_lo->parttimepercentage, 2);
					}
					if (strlen($i_lo->i_func->naam) > 0) {
						$oms .= " (" . $i_lo->i_func->naam . ")";
					}

					if ($i_lo->vanaf > $this->i_rk->i_sz->begindatum) {
						$oms .= " vanaf " . date("d-m-Y", strtotime($i_lo->vanaf));
					}

					if ($p_uitvoeren == 1) {
						$rrid = $this->add($p_rkid, $i_lo->lidid, $kpl);
						if ($rrid > 0) {
							$this->update($rrid, "LidondID", $lorow->RecordID);
							$this->update($rrid, "ActiviteitID", $i_lo->i_gr->actid);
							$this->update($rrid, "OMSCHRIJV", $oms);
							$this->update($rrid, "Bedrag", $cb);
							if (function_exists("fnMaatwerkNaToevoegenStandaardRegelOpRekening")) {
								fnMaatwerkNaToevoegenStandaardRegelOpRekening($rrid);
	...
	public function add($p_rkid, $p_lidid=-1, $p_kpl="") {
		...
		if ($rnr < 1) {
			...
			$query = sprintf("INSERT INTO %s (RecordID, Rekening, Lid, Regelnr, KSTNPLTS) VALUES (:recordid, :rekening, :lid, :regelnr, :kstnplts);", $this->table);
			if ($dbc->prepare($query)->execute($data) > 0) {
				$nrid = $data['recordid'];
				$this->mess = sprintf("Tabel Rekreg: regel %d is aan rekening %d toegevoegd", $rnr, $this->rkid);
			} else {
				$this->mess = sprintf("Tabel Rekreg: regel %d is niet aan rekening %d toegevoegd (fout bij toevoegen)", $rnr, $this->rkid);
			}
		} else {
			$this->mess = "Je bent niet bevoegd om een rekeningregel toe te voegen.";
		}
...
class cls_Seizoen extends cls_db_base {
	...
	public string $rekeningomschrijving = "";
	public int $gezinsrekening = 0;
	public $verenigingscontributieomschrijving = "";
	public $verenigingscontributiekostenplaats = "";
	public int $afdelingscontributieomschrijving = 1;
	public float $contributiekader = 0;
	public float $contributielid = 0;
	public float $contributiejeugdlid = 0;
	public float $gezinskorting = 0;
	public string $gezinskortingomschrijving = "";
	public string $gezinskortingkostenplaats = "";
	public int $betaaldagentermijn = 30;
	...
	public function vulvars($p_szid=-1) {
		...
		if ($this->szid > 0) {
			...
			if (isset($szrow->Nummer)) {
				...
				$this->gezinsrekening = $szrow->RekeningenVerzamelen ?? 0;
				...
				$this->gezinskorting = $szrow->{'Gezinskorting bedrag'} ?? 0;
				$this->gezinskortingomschrijving = $szrow->{'Gezinskorting omschrijving'} ?? "";
				$this->gezinskortingkostenplaats = $szrow->{'Gezinskorting kostenplaats'} ?? "";
	...
	}  # cls_Seizoen->controle
	
	public function opschonen() {
		
		$this->optimize();
		
	}  # cls_Seizoen->opschonen
...
class cls_Eigen_lijst extends cls_db_base {
	...
	public function vulvars($p_elid=-1, $p_zoek="") {
		if ($p_elid >= 0) {
			$this->elid = $p_elid;
		}
		...
		$this->mysql = "";
		$this->eigenscript = "";
		$this->aantalkolommen = 0;
		$this->aantalrijen = 0;
		$this->tabpage = "";
		
		if ($p_elid <= 0 and strlen($p_zoek) > 0) {
			$query = sprintf("SELECT IFNULL(MAX(EL.RecordID), 0) FROM %s WHERE %s;", $this->basefrom, $p_zoek);
			$this->elid = $this->scalar($query);
		}
		...
		if ($this->elid > 0) {
			...
			if (isset($elrow->RecordID)) {
				...
				$this->gewijzigd = $elrow->Gewijzigd ?? "";
				
				$this->vularrayparams($this->waarde_params);
			} else {
				$this->elid = 0;
			}
			...
			if (strlen($this->eigenscript) > 0 and file_exists(BASEDIR . "/maatwerk/" . $this->eigenscript) == false) {
				$this->eigenscript = "";
			}
			
			if (strlen($this->tabpage) > 0) {
				$this->url = sprintf("%s?tp=%s/%s", $_SERVER['PHP_SELF'], $this->tabpage, str_replace(" ", "%20", $this->naam));
			} else {
				$this->url = "";
			}
		}
	...
	public function vularrayparams($p_waarden) {
		$this->aantal_params = 0;
		$this->alleparamsgevuld = true;
		$arrWaardes = explode(";", $p_waarden);
		for ($p=1;$p<=9;$p++) {
			if (strpos($this->mysql, "@P" . $p) > 0) {
				$this->aantal_params++;
				if (count($arrWaardes) > $p) {
					$this->array_params[$p] = $arrWaardes[$p-1];
				}
				if (strlen($this->array_params[$p]) == 0) {
					$this->alleparamsgevuld = false;
				}
			}
		}
	}  # cls_Eigen_lijst->vularrayparams
	...
	public function lijst($p_filter="") {
		...
		if ($p_filter === 1) {
			...
			$w = "WHERE EL.AantalKolommen > 0 AND EL.KolomLidID >= 0 AND EL.Aantal_params=0";
		} elseif ($p_filter === 3) {
			// Alleen eigen lijsten die records hebben en geschikt zijn voor selecties/filters/mailingen
			$w = "WHERE EL.AantalRecords > 0 AND EL.AantalKolommen > 0 AND EL.KolomLidID >= 0 AND EL.Aantal_params=0";
		} elseif ($p_filter == 4) {
			// Persoonlijke meldingen op het voorblad
			$w = sprintf("WHERE EL.AantalKolommen > 0 AND EL.GroepMelding > 0 AND EL.GroepMelding IN (%s)", $_SESSION['lidgroepen']);
	...
	public function mysql($p_elid=-1, $p_as_sub=0) {
		...
		return $rv;
	}  # cls_Eigen_lijst->mysql
	
	public function rowset($p_elid=-1, $p_fetched=1) {
		global $dbc;
	...
	public function add() {
		...
		if (WEBMASTER)  {
			$query = sprintf("INSERT INTO %s (RecordID, Naam, MySQL, AantalRecords, AantalKolommen, Ingevoerd) VALUES (%d, '*** Nieuwe lijst ***', '', 0, 0, NOW());", $this->table, $this->nieuwrecordid());
			$elid = $this->execsql($query);
			$this->mess = sprintf("Tabel Eigen_lijst: record %d toegevoegd.", $elid);
		} else {
			$elid = 0;
			$this->mess = "Je bent niet bevoegd om een eigen lijst toe te voegen.";
		}
	...
	public function delete($p_elid) {
		...
		if (WEBMASTER)  {
			...
			$this->mess = "Je bent niet bevoegd om een eigen lijst toe te verwijderen.";
		}
		$this->log($p_elid, 1);
	}  # cls_Eigen_lijst->delete
	
	public function controle($p_elid=-1, $p_interval=180, $p_maxaantal=5) {
		global $dbc;
		...
		if ($p_maxaantal <= 0) {
			$p_maxaantal = intval($this->aantal()/6);
		}

		if ($p_elid > 0 and $p_interval > 0) {	
			$w = sprintf("EL.RecordID=%d AND EL.LaatsteControle < DATE_SUB(NOW(), INTERVAL %d MINUTE)", $p_elid, $p_interval);
		} elseif ($p_elid > 0) {
			$w = sprintf("EL.RecordID=%d", $p_elid);
		} else {
			$w = sprintf("EL.LaatsteControle < DATE_SUB(NOW(), INTERVAL %d MINUTE)", $p_interval);
		}
		
		$query = sprintf("SELECT * FROM %s WHERE %s ORDER BY EL.LaatsteControle", $this->basefrom, $w);
		if ($p_maxaantal > 0) {
			$query .= sprintf(" LIMIT %d", $p_maxaantal);
		}
		$query .= ";";
		$result = $this->execsql($query);
		...
		foreach ($elrows as $row) {
			...
			} elseif (strlen($this->eigenscript) > 4 and file_exists(BASEDIR . "/maatwerk/" . $this->eigenscript) == false) {
				$this->mess = sprintf("%s bestaat niet.", $this->eigenscript);
				$this->tas = 11;
				$this->Log($row->RecordID);
			}
			...
			$rv++;
			if (strlen($gecontroleerd) > 0) {
				$gecontroleerd .= ', ';
			}
			$gecontroleerd .= $row->RecordID;
		}
		...
		if (count($elrows) > 0) {
			if (count($elrows) > 1) {
				$this->mess = sprintf("%s->%s er zijn in %.2f seconden %d lijsten (%s) gecontroleerd.", __CLASS__, __FUNCTION__, $exec_tijd, $rv, $gecontroleerd);
				$this->log();
			} else {
				$this->mess = sprintf("%s->%s in %.2f seconden is eigen lijst %d '%s' gecontroleerd.", __CLASS__, __FUNCTION__, $exec_tijd, $this->elid, $this->naamlogging);
				$this->log($this->elid);
			}
		}
...
class cls_Foto extends cls_db_base {
	...
	private function vulvars($p_ftid=-1, $p_lidid=-1) {
		...
		if ($this->lidid > 0 and $this->ftid <= 0) {
			$query = sprintf("SELECT IFNULL(MAX(Foto.RecordID), 0) FROM %s WHERE Foto.LidID=%d;", $this->basefrom, $this->lidid);
	...
	public function opschonen() {
		...
		if ($bt > 1) {
			...
			foreach ($this->basislijst() as $frow) {
				$this->vulvars($frow->RecordID);
				if (strlen($this->i_lid->eindelaatstelidmaatschap) == 10 and $this->i_lid->eindelaatstelidmaatschap < $hd) {
		...
		foreach ($this->execsql($query)->fetchAll() as $frow) {
			$this->delete($frow->RecordID, "het lid niet bestaat.");
		}
	...
	public function vulvars($p_insid=-1) {
		...
		if ($this->insid > 0) {
			...
			if (isset($row->Naam)) {
				...
				if ($row->PDF == null) {
					$this->pdfaanwezig = false;
				} else {
					$this->pdfaanwezig = true;
				}
...
class cls_Template extends cls_db_base {
	...
	public function controle() {
		...
		$i_sz->where = "SZ.Einddatum > CURDATE() AND `Contributie leden` > 0";
		foreach ($i_sz->basislijst() as $row) {
			$arrTP[] = sprintf("rekening %d", $row->Nummer);
		}
	...
	public function vulsessie() {
		foreach ($this->lijst() as $row) {
			if (($row->ParamType == "F" or $row->ParamType == "I") and empty($row->ValueNum)) {
				...
				$_SESSION['settings'][$row->Naam] = $row->ValueNum ?? 0;
			} elseif ($row->ParamType == "B" or $row->ParamType == "I") {
				$_SESSION['settings'][$row->Naam] = intval($row->ValueNum ?? 0);
			} else {
...
function db_onderhoud($type=9) {
	...
	*/
	...
	// Deze code kan na 1 januari 2025 worden verwijderd.
	$tab = TABLE_PREFIX . "Onderdl";
	$col = "AfmeldenMogelijk";
	if ($i_base->bestaat_kolom($col, $tab) == false) {
		$query = sprintf("ALTER TABLE `%s` ADD `%s` TINYINT NULL DEFAULT '0' AFTER `Alleen leden`;", $tab, $col);
		$i_base->execsql($query, 2);
	}
	
	// Deze code kan na 1 april 2025 worden verwijderd.
	$tab = TABLE_PREFIX . "Evenement";
	$col = "MaxDeelnemers";
	if ($i_base->bestaat_kolom($col, $tab) == false) {
		$query = sprintf("ALTER TABLE `%s` ADD `%s` INT NULL DEFAULT '0' COMMENT 'Maximale aantal deelnemers aan dit evenement' AFTER `StandaardStatus`;", $tab, $col);
		$i_base->execsql($query, 2);
	}
	...
	// Deze code kan na 1 augustus 2025 worden verwijderd.
	$tab = TABLE_PREFIX . "Mailing_vanaf";
	$col = "VervallenPer";
	if ($i_base->bestaat_kolom($col, $tab) == false) {
		$query = sprintf("ALTER TABLE `%s` ADD `%s` DATE NULL AFTER `SMTP_server`;", $tab, $col);
		$i_base->execsql($query, 2);
	}
	
	// Deze code kan na 1 augustus 2025 worden verwijderd.
	$tab = TABLE_PREFIX . "Admin_activiteit";
	$idx = "TableLidIDReferID";
	if ($i_base->bestaat_index($tab, $idx) == false) {
		$query = sprintf("ALTER TABLE `%s` ADD INDEX %s (`RefTable`, `LidID`, `ReferID`);", $tab, $idx);
		$i_base->execsql($query, 2);
	}
	...
	// Deze code mag na 1 mei 2025 worden verwijderd.
	$query = sprintf("ALTER TABLE `%sRekreg` CHANGE `Lid` `Lid` INT(11) NULL DEFAULT '0';", TABLE_PREFIX);
	$i_base->execsql($query);
	
	$query = sprintf("ALTER TABLE `%sRekreg` CHANGE `Bedrag` `Bedrag` DECIMAL(8,2) NULL DEFAULT '0';", TABLE_PREFIX);
	$i_base->execsql($query);
	
	$query = sprintf("ALTER TABLE `%sRekreg` CHANGE `ToonOpRekening` `ToonOpRekening` TINYINT(4) NULL DEFAULT '1';", TABLE_PREFIX);
	$i_base->execsql($query);
