...
class cls_Lid extends cls_db_base {
	...
	public function lidbijemail($p_email) {
		...
		$query = sprintf("SELECT L.RecordID AS LidID, LM.Lidnr, %1\$s AS NaamLid FROM %2\$s INNER JOIN %3\$sLidmaatschap AS LM ON L.RecordID=LM.Lid
						  ...
						  ORDER BY LM.Lidnr;", $this->selectnaam, $this->basefrom, TABLE_PREFIX, $p_email);
...
class cls_Lidond extends cls_db_base {
	...
	private function vulvars($p_loid=-1, $p_lidid=-1, $p_ondid=-1, $p_per="") {
		...
		if ($this->loid > 0) {
			$this->query = sprintf("SELECT LO.Lid, LO.OnderdeelID, LO.GroepID, LO.Vanaf FROM %s WHERE LO.RecordID=%d;", $this->basefrom, $this->loid);
...
class cls_Liddipl extends cls_db_base {
	...
	public function dubbelediplomas($p_orgldid) {
		...
		if (isset($row->RecordID)) {
			...
			$query = sprintf("SELECT LD.DatumBehaald FROM %s WHERE LD.DatumBehaald < '%s' AND LD.Lid=%d AND LD.DiplomaID=%d ORDER BY LD.DatumBehaald;", $this->basefrom, $behaaldvoor, $lidid, $dpid);
			$result = $this->execsql($query);
...
class cls_Examen extends cls_db_base {
	...
	public function vulvars($p_exid) {
		if ($p_exid >= 0) {
			$this->exid = $p_exid;
		}
		if ($this->exid > 0) {
	...
	public function opschonen() {
		$query = sprintf("SELECT EX.Nummer FROM %s WHERE EX.Ingevoerd < DATE_SUB(CURDATE(), INTERVAL %d DAY) AND (SELECT COUNT(*) FROM %sLiddipl AS LD WHERE LD.Examen=EX.Nummer)=0;", $this->basefrom, BEWAARTIJDNIEUWERECORDS, TABLE_PREFIX);
		$result = $this->execsql($query);
		$reden = "het examen in het verleden ligt en er een gekoppelde records (meer) zijn.";
		foreach ($result->fetchAll() as $exrow) {
			if ($exrow->Datum < date("Y-m-d")) {
	...
	public function lijst($p_soort=1, $p_datum="", $p_filter="") {
		...
		*/
		...
		if ($p_soort == 2) {
			...
			$where = sprintf("E.Datum > DATE_SUB(NOW(), INTERVAL 36 HOUR) AND IFNULL(E.VerwijderdOp, '1900-01-01') < '2012-01-01' AND E.BeperkTotGroep IN (%s)", $_SESSION["lidgroepen"]);
			$ord = "E.Datum";
...
class cls_Rekening extends cls_db_base {	
	...
	public function add($p_rkid, $p_seiz, $p_lidid) {
		...
		if ($this->lidid <= 0) {
			...
			$this->query = sprintf("INSERT INTO %1\$s (%2\$s, Seizoen, Datum, Lid, BetaaldDoor, BET_TERM, Betaald, Ingevoerd) VALUES (%3\$d, %4\$d, CURDATE(), %5\$d, %6\$d, 1, 0, CURDATE());", $this->table, $this->pkkol, $nwreknr, $p_seiz, $this->lidid, $i_lid->rekeningbetaalddoor);
		
			if ($this->execsql() > 0) {
				$this->mess = sprintf("Rekening %d voor lid %d is toegevoegd.", $nwreknr, $this->lidid);
				$this->log($nwreknr);
			
				$sql = sprintf("INSERT INTO Rekening (%1\$s, Seizoen, Datum, Lid, BetaaldDoor, BET_TERM, Betaald, Ingevoerd) VALUES (%2\$d, %3\$d, CURDATE(), %4\$d, %5\$d, 1, 0, CURDATE());", $this->pkkol, $nwreknr, $p_seiz, $this->lidid, $i_lid->rekeningbetaalddoor);
				$this->Interface($sql);
	...
	public function opschonen() {
		...
		$query = sprintf("SELECT RK.Nummer FROM %s WHERE RK.Ingevoerd < DATE_SUB(NOW(), INTERVAL 6 HOUR) AND (RK.Nummer NOT IN (SELECT RR.Rekening FROM %sRekreg AS RR)) LIMIT 100;", $this->basefrom, TABLE_PREFIX);
		$result = $this->execsql($query);
		$reden = "de rekening geen regels (meer) heeft.";
		foreach ($result->fetchAll() as $rkrow) {
