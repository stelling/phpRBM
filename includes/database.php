...
//							$mess = sprintf("Uitgevoerd zonder resultaat: %s", $this->query);
	...
	public function typekolom($p_kolom, $p_table="") {
		...
		} elseif ($p_kolom == "LIDDATUM" or $p_kolom == "Opgezegd" or $p_kolom == "Vanaf" or $p_kolom == "DatumBehaald" or $p_kolom == "Einddatum" or $p_kolom == "Datum") {
		...
		if ($p_lidid != $this->lidid) {
			...
			if ($this->lidid > 0) {
				...
				if (isset($row->RecordID) and $row->RecordID > 0) {
					...
					if ($row->RekeningBetaaldDoor > 0) {
						$this->rekeningbetaalddoor = $row->RekeningBetaaldDoor;
					} else {
						$this->rekeningbetaalddoor = $this->lidid;
					}
...
class cls_Authorisation extends cls_db_base {
	...
	public function opschonen() {
		$query = sprintf("SELECT RecordID FROM %s WHERE AA.LaatstGebruikt < DATE_ADD(CURDATE(), INTERVAL -3 MONTH) AND Toegang < 0;", $this->basefrom);
		$result = $this->execsql($query);
...
class cls_Lidond extends cls_db_base {
	...
	public function vulvars($p_loid=-1, $p_lidid=-1, $p_ondid=-1, $p_per="") {
		...
		if ($this->vanaf > date("Y-m-d")) {
			...
			$this->lotitle = sprintf("heeft per %s opgezegd", $row->Opgezegd);
		}
...
// Logging, waar geen lid aan is gekoppeld.
		$query = sprintf("DELETE FROM %s WHERE DatumTijd < DATE_SUB(CURDATE(), INTERVAL 18 MONTH) AND ReferLidID=0;", $this->table);
		$this->execsql($query, 2);
...
class cls_Liddipl extends cls_db_base {
	...
	public function opschonen() {
		...
		$query = sprintf("SELECT LD.RecordID, DP.HistorieOpschonen FROM %s INNER JOIN %sDiploma AS DP ON LD.DiplomaID=DP.RecordID
								WHERE IFNULL(LD.LicentieVervallenPer, '9999-12-31') < DATE_SUB(CURDATE(), INTERVAL DP.HistorieOpschonen MONTH);", $this->basefrom, TABLE_PREFIX);
		$result = $this->execsql($query);
		foreach ($result->fetchAll() as $row) {
		...
		*/
		...
		if ($p_soort == 2) {
			...
			if (strlen($p_datum) == 10) {
				$where = sprintf("E.Datum='%s'", $p_datum);
			} else {
...
class cls_Evenement_Deelnemer extends cls_db_base {
	...
	public function opschonen() {
		...
		$query = sprintf("SELECT ED.RecordID FROM %s WHERE (ED.Status='G' OR LENGTH(ED.Status)=0) AND LENGTH(IFNULL(ED.Opmerking, ''))=0;", $this->basefrom);
		$result = $this->execsql($query);
		foreach ($result->fetchAll() as $row) {
			$this->delete($row->RecordID, "de status geen is en er geen opmerking is ingevuld.");
		}
	...
	private function vulvars($p_rkid=-1) {
		...
		if ($this->rkid > 0) {
			...
			if (isset($rkrow->Nummer) and $rkrow->Nummer > 0) {
				...
				$this->naamlogging = $rkrow->DEBNAAM;
		...
		if ($this->lidid > 0) {
			$lrow = (new cls_Lid())->record($this->lidid);
			$this->adres = $lrow->Adres;
			$this->postcode = $lrow->Postcode;
		} else {
			$this->adres = "";
			$this->postcode = "";
		}
