...
class cls_Lidond extends cls_db_base {
	...
	public function vulvars($p_loid=-1, $p_lidid=-1, $p_ondid=-1, $p_per="") {
		...
		if ($this->groepid > 0 and $this->i_gr->dpid > 0) {
			...
			$f = sprintf("AK.Datum > '%s' AND AK.Datum < CURDATE()", $startdatumvoortgang);
...
class cls_Groep extends cls_db_base {
	...
	public function lijst($p_afdid=-1, $p_order="GR.Starttijd, GR.Volgnummer, GR.Omschrijving, GR.RecordID") {
		...
		$query = sprintf("SELECT GR.*
								...
								ORDER BY %4\$s;", TABLE_PREFIX, $this->basefrom, $this->afdid, $p_order);
		$result = $this->execsql($query);
		return $result->fetchAll();		
	}
...
class cls_Artikel extends cls_db_base{
	...
	public function add($p_code="") {
		...
		$query = sprintf("INSERT INTO %s (RecordID, Code, MaxAantalPerLid) VALUES  (%d, '%s', 1);", $this->table, $nrid, $p_code);
		if ($this->execsql($query) > 0) {
